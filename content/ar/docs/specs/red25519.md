...
description: "مخطط التوقيع القابل لإعادة العشوائية لإنشاء Destinations مموهة"
slug: "red25519"
aliases:
  - "/ar/docs/specs/red25519-signature-scheme"
  - "/ar/docs/specs/red25519-signature-scheme/"
category: "التصميم"
lastUpdated: "2020-08"
accurateFor: "0.9.47"
...
تحدد هذه الوثيقة مخطط توقيع قابل لإعادة العشوأة مناسب لإنشاء Destinations يمكن إخفاؤها. يمكن استخدامه إضافياً لإخفاء Ed25519 Destinations الموجودة، مع انخفاض طفيف في الفعالية.

## الدافع

[الاقتراح 123](/proposals/123-new-netdb-entries) يحدد تنسيق LeaseSet2 مشفر يجسد مبدأ أقل صلاحية: يُمنح كل مشارك في الشبكة فقط المعلومات الضرورية لدوره. على وجه الخصوص، LeaseSet2 مشفر منشور إلى floodfill لا يكشف عن الوجهة (Destination) التي يخصها، ولا يمكن عرض الـ Leases إلا من قبل شخص لديه معرفة مسبقة بالوجهة. ومع ذلك، لا تزال floodfills بحاجة إلى أن تكون قادرة على مصادقة LeaseSet2s المشفرة عند نشرها، والعملاء بحاجة إلى التأكد بالإضافة إلى ذلك من أن المصادقة تم فرضها من قبل الوجهة نفسها.

يحقق الاقتراح 123 هذا من خلال إخفاء مفاتيح التوقيع الخاصة بـ Destinations. يمكن استخدام المفاتيح المُخفاة لإنشاء توقيعات قابلة للتحقق بواسطة floodfills، ويمكن للعملاء أن يكونوا متأكدين من أن Destination فقط هو الذي يمكنه إنشاء هذه التوقيعات. لذلك من الضروري تحديد مخطط توقيع يمكن استخدامه للإخفاء.

## التصميم

### نظام التوقيع الأساسي

مخطط التوقيع المحدد هنا، Red25519، هو تطبيق لـ [RedDSA](https://github.com/zcash/zips/tree/master/protocol/protocol.pdf)، وهو مخطط توقيع مبني على Schnorr يدعم إعادة عشوأة المفاتيح. يحتوي على الوظائف التالية:

**GENERATE_PRIVATE()** : إرجاع مفتاح خاص عشوائي موزع بانتظام.

**DERIVE_PUBLIC(sk)** : يُرجع المفتاح العام المقابل للمفتاح الخاص المُعطى.

**GENERATE_RANDOM()** : يُرجع قيمة عشوائية مناسبة لإعادة عشوأة زوج المفاتيح.

**RANDOMIZE_PRIVATE(sk, alpha)** : يعيد عشوأة المفتاح الخاص، باستخدام قيمة قياسية سرية alpha.

**RANDOMIZE_PUBLIC(vk, alpha)** : يعيد عشوأة المفتاح العام، باستخدام قيمة عددية سرية alpha.

**SIGN(sk, m)** : يُرجع توقيعاً بالمفتاح الخاص sk على الرسالة المعطاة m.

**VERIFY(vk, m, sig)** : يتحقق من التوقيع sig مقابل المفتاح العام vk والرسالة m. يُرجع true إذا كان التوقيع صالحاً، وإلا فيُرجع false.

لزوج مفاتيح معين (sk, vk) تكون العلاقة التالية صحيحة:
...
مفاتيح Ed25519 يمكن تحويلها مؤقتاً في اتجاه واحد إلى مفاتيح Red25519، لدعم إعادة العشوائية للوجهات Ed25519 الموجودة. أنواع التوقيع الأخرى غير متوافقة.

نحن نحدد دوال التحويل التالية:

**CONVERT_ED25519_PRIVATE(privkey)** : يُرجع المفتاح الخاص Red25519 المقابل للمفتاح الخاص Ed25519 المُعطى.

**CONVERT_ED25519_PUBLIC(pubkey)** : يُرجع مفتاح Red25519 العام المقابل لمفتاح Ed25519 العام المُعطى.

لزوج مفاتيح Ed25519 معطى (privkey, pubkey) تسري العلاقة التالية:
...
**B** : نقطة الأساس Ed25519 كما في [RFC-8032](https://tools.ietf.org/html/rfc8032).

**L** : ترتيب Ed25519 هو 2^252 + 27742317777372353535851937790883648493 كما هو موضح في [RFC-8032](https://tools.ietf.org/html/rfc8032).

**[s] B** : ضرب قياسي ثابت القاعدة للنقطة الأساسية بـ s.

**[s] A** : ضرب عددي متغير القاعدة لـ A بواسطة s.

**x || y** : ربط مصفوفتي البايت x و y.

### Red25519

يتخصص المخطط Red25519 في RedDSA مع:

- G := مجموعة النقاط على شكل Edwards للمنحنى Curve25519. وهذا يعني على وجه التحديد أن Red25519 يستخدم المجموعة الفرعية ذات الرتبة الأولية L، والعامل المشترك h_G يساوي 8.
- P_G := النقطة الأساسية Ed25519 وهي B.
- l_H := 512
- H(x) := SHA-512("I2P_Red25519H(x)" || x)

يفترض RedDSA أن H(x) يتم تطبيقه باستخدام دالة تشفير hash آمنة ضد هجمات تمديد الطول. SHA-512 لا يحقق هذا بمفرده. لمعالجة هذا، نتطلب أن تكون الرسائل مسبوقة بترميز خالٍ من البادئة لطولها:

```
len_u16(M) || M
```
حيث `len_u16(M)` هو التمثيل المكون من بايتين لطول M، بترتيب البايت الأقل أهمية أولاً (ليكون متسقاً مع ترميز البايت الأقل أهمية أولاً للقيم العددية والنقاط).

يجب أن تكون الرسائل لا تزيد عن 65534 بايت. طول 65535 محجوز للامتدادات المستقبلية المحتملة.

### الترميز وفك الترميز

مفاتيح Red25519 الخاصة هي scalars mod L، مُرمزة بتمثيل little-endian. نحن نُعرِّف الدوال `DECODE_SCALAR` و `ENCODE_SCALAR` للتبديل بين صيغة مصفوفة البايت والصيغة الصحيحة للـ scalar.

مفاتيح Red25519 العامة هي نقاط على الشكل Edwards لمنحنى Curve25519. يتم ترميزها كتمثيل 255-بت little-endian للإحداثية y، متبوعة ببت واحد يشير إلى إشارة الإحداثية x. هذا هو نفس الترميز المستخدم في Ed25519. نحدد الدالتين `DECODE_POINT` و `ENCODE_POINT` للتبديل بين شكل مصفوفة البايت وشكل الإحداثيات للنقطة.

### دوال RedDSA

لسهولة التنفيذ، نكتب صراحة أدناه دوال RedDSA (بالإضافة إلى عدة دوال مساعدة) المخصصة بالفعل لـ Red25519. يجب على المطورين الرجوع إلى القسم 5.4.6 من [RedDSA](https://github.com/zcash/zips/tree/master/protocol/protocol.pdf) للمواصفات العامة لدوال RedDSA.
...
```
لاحظ أن تطبيق `CONVERT_ED25519_PRIVATE` مكافئ لحساب العدد القياسي السري s عند اشتقاق مفتاح Ed25519 العام من مفتاح Ed25519 الخاص، كما هو محدد في الخطوات 1-3 من القسم 5.1.5 من [RFC-8032](https://tools.ietf.org/html/rfc8032).

## الآثار الأمنية

إعادة عشوأة وجهة Red25519 ثم إنشاء التوقيعات معها لا يكشف أي معلومات حول الوجهة، لأن توزيع المفاتيح الخاصة لـ Red25519 المُولدة عبر `RANDOMIZE_PRIVATE` مطابق لتوزيع المفاتيح الخاصة المُولدة عبر `GENERATE_PRIVATE`، و `DERIVE_PUBLIC` حتمي.

تحويل المفاتيح الخاصة Ed25519 إلى Red25519 عبر `CONVERT_ED25519_PRIVATE` لا ينتج عنه نفس التوزيع. ومع ذلك، نعتبر التقليل في الأمان مقبولاً للأسباب التالية:

- مساحة Ed25519 scalars تقريباً نصف حجم مساحة Red25519 scalars (هناك 2^251 من Ed25519 scalars الممكنة، و L ~= 2^252 من Red25519 scalars الممكنة). وبالتالي فإن فقدان الأمان لا يزيد عن عامل 2 تقريباً، أو حوالي 1 بت (لأنه كان بإمكاننا أن نختار بالصدفة Red25519 scalar صالح أيضاً كـ Ed25519 scalar).
- وجهات Ed25519 الموجودة كانت معرضة تاريخياً على الشبكة، ويجب افتراض أن floodfills الخبيثة قد قامت بالفعل بإحصائها.

المستخدمون الذين يشعرون بالقلق حول هذا التقليل في الأمان يجب أن يستخدموا Red25519 كنوع التوقيع (sigtype) لوجهاتهم (Destinations) بدلاً من Ed25519.

لاحظ أن الحجة المذكورة أعلاه لا تنطبق على القيمة العددية alpha الخاصة بإعادة العشوائية؛ يتم تسريب معلومات حول المفتاح في كل مرة يتم اختيار alpha منحاز، لأن إعادة العشوائية الجمعية تتصرف مثل الرقعة المستخدمة مرة واحدة.

## التوافق

إصدارات I2P التي تدعم Red25519 ستكون قادرة على التحقق من هياكل البيانات الشبكية الموقعة به. إصدارات I2P التي لا تدعم Red25519 ستتعامل معه كتوقيع غير معروف، وقد تتجاهل هياكل البيانات. يجب على المستخدمين توقع أن تكون موثوقية هياكل البيانات الموقعة بـ Red25519 ضعيفة حتى يتم تحقيق نشر كافٍ لـ Red25519.
...
- [اقتراح 123](/proposals/123-new-netdb-entries)
