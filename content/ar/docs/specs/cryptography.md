...
راجع كود ElGamal ElGamalEngine.
...
نستخدم AES مع مفاتيح 256 بت وكتل 128 بت في وضع CBC. الحشو المستخدم محدد في IETF [RFC-2313](http://tools.ietf.org/html/rfc2313) (PKCS#5 1.5، القسم 8.1 (لنوع الكتلة 02)). في هذه الحالة، يتكون الحشو من ثمانيات بايت مُولدة عشوائياً كاذباً لتطابق كتل 16 بايت. على وجه التحديد، راجع كود CBC CryptixAESEngine وتنفيذ Cryptix AES CryptixRijndael_Algorithm، بالإضافة إلى الحشو الموجود في دالة ElGamalAESEngine.getPadding ElGamalAESEngine.
...
يتم إنشاء التوقيعات والتحقق منها باستخدام [DSA](http://en.wikipedia.org/wiki/Digital_Signature_Algorithm) بحجم 1024 بت (L=1024, N=160)، كما هو مُطبق في DSAEngine. تم اختيار DSA لأنه أسرع بكثير للتوقيعات من ElGamal.
