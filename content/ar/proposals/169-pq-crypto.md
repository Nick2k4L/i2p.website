...
| NTCP2 | تجريبي في الربع الأول 2026، إصدار في الربع الثاني 2026 |
| SSU2 | التنفيذ جارٍ، تجريبي في الربع الثاني 2026، إصدار في الربع الثالث 2026 |
| MLDSA SigTypes | متوقف مؤقتًا حتى 2027-2028، انظر [PLANTS](https://datatracker.ietf.org/wg/plants/about/) |
## نظرة عامة
...
ملاحظة: جميع المعلومات في هذا الاقتراح المتعلقة بتوقيعات MLDSA أولية. العمل على دعم توقيعات MLDSA في I2P متوقف حتى أواخر عام 2027 أو 2028، انتظارًا لأعمال الهيئات القياسية لاختيار الخوارزميات، وربما تقليل أحجام المفاتيح و/أو التوقيعات، وتعزيز اعتماد الصناعة. انظر [CABFORUM](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/) و [PLANTS](https://datatracker.ietf.org/wg/plants/about/).

سنقوم بدعم التوقيعات الكمية (PQ) والتوقيعات الهجينة في الهياكل التالية:
...
لذلك سنقوم بدعم التوقيعات الكمية فقط (PQ-only) والتوقيعات الهجينة على حد سواء. سنُعرّف ثلاثة أنواع من متغيرات ML-DSA كما هو مذكور في [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf)، وثلاثة متغيرات هجينة مع Ed25519، وثلاثة متغيرات كمية فقط مع ما قبل التجزئة (prehash) للملفات من نوع SU3 فقط، ليصبح المجموع 9 أنواع جديدة من التوقيعات. سيتم تعريف الأنواع الهجينة فقط بالاشتراك مع Ed25519. سنستخدم ML-DSA القياسي، وليس المتغيرات التي تستخدم ما قبل التجزئة (HashML-DSA)، باستثناء ملفات SU3.

سوف نستخدم التنوع "المحمي" أو التوقيع العشوائي، وليس التنوع "الحتمي"، كما هو محدد في القسم 3.4 من [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf). ويضمن هذا أن يكون كل توقيع مختلفًا، حتى عند تطبيقه على نفس البيانات، ويوفر حماية إضافية ضد هجمات القنوات الجانبية. انظر قسم ملاحظات التنفيذ أدناه للحصول على تفاصيل إضافية حول خيارات الخوارزمية بما في ذلك الترميز والسياق.
...
| MLDSA65ph | 19 |
| MLDSA87ph | 20 |
ستستخدم شهادات X.509 والترميزات الأخرى بصيغة DER الهياكل المركبة ومعرفات الكائنات (OIDs) المعرّفة في [مسودة IETF](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/).

ستكون التكلفة الإضافية كبيرة. حجم الهوية النموذجية لوجهة Ed25519 وهوية الموجه هو 391 بايت. وسوف يزداد هذا الحجم بعامل يتراوح بين 3.5 إلى 6.8 حسب الخوارزمية. التوقيعات Ed25519 هي 64 بايت. وستزداد هذه التوقيعات بعامل يتراوح بين 38 إلى 76 حسب الخوارزمية. الرسائل النموذجية الموقعة مثل RouterInfo وLeaseSet وحزم البيانات القابلة للإجابة والرسائل المتسلسلة الموقعة هي حوالي 1 كيلوبايت. وستزداد هذه الرسائل بعامل يتراوح بين 3 إلى 8 حسب الخوارزمية.

بما أن أنواع الوجهة وهوية الموجه الجديدة لن تحتوي على حشو، فلن تكون قابلة للضغط. وستزداد أحجام الوجهات وهويات الموجه التي يتم ضغطها باستخدام gzip أثناء النقل بنسبة تتراوح بين 12 إلى 38 مرة حسب الخوارزمية المستخدمة.

### التركيبات القانونية

بالنسبة للوجهات، يتم دعم أنواع التوقيع الجديدة مع جميع أنواع التشفير في leaseset. اضبط نوع التشفير في شهادة المفتاح على NONE (255).

نوع تشفير ElGamal مُهمَل بالنسبة لهويات الموجه (RouterIdentities). يتم دعم أنواع التوقيع الجديدة مع تشفير X25519 (النوع 4) فقط. سيتم الإشارة إلى أنواع التشفير الجديدة في عناوين الموجه (RouterAddresses). سيظل نوع التشفير في شهادة المفتاح هو النوع 4.
...
متجهات الاختبار لـ SHA3-256 وSHAKE128 وSHAKE256 متاحة على [NIST](https://csrc.nist.gov/projects/cryptographic-standards-and-guidelines/example-values).

لاحظ أن مكتبة Java bouncycastle تدعم جميع ما سبق. توجد دعم المكتبة C++ في OpenSSL 3.5 [OpenSSL](https://openssl-library.org/post/2025-02-04-release-announcement-3.5/).

### البدائل

لن ندعم [FIPS 205](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.205.pdf) (Sphincs+)، لأنه أبطأ بكثير وأكبر حجمًا من ML-DSA. ولن ندعم FIPS206 القادم (Falcon)، لأنه لم يتم توحيد معاييره بعد. ولن ندعم NTRU أو أي مرشحات أخرى للحوسبة الكمية لم تُوحَّد بواسطة NIST.

### Rosenpass

توجد بعض الأبحاث [ورقة بحثية](https://eprint.iacr.org/2020/379.pdf) حول تكييف Wireguard (IK) للتشفير الكمي البحت (PQ crypto)، ولكن هناك عدة أسئلة مفتوحة في تلك الورقة. لاحقًا، نُفذ هذا النهج على شكل Rosenpass [Rosenpass](https://rosenpass.eu/) [الورقة البيضاء](https://raw.githubusercontent.com/rosenpass/rosenpass/papers-pdf/whitepaper.pdf) لتوفير Wireguard مع حماية كمية (PQ Wireguard).

يستخدم Rosenpass عملية بدء اتصال تشبه بروتوكول Noise KK مع مفاتيح ثابتة من نوع McEliece الكلاسيكية 460896 مشتركة مسبقًا (بحجم 500 كيلوبايت لكل منها) ومفاتيح عابرة من نوع Kyber-512 (وهي في جوهرها MLKEM-512). وبما أن النصوص المشفرة من نوع McEliece الكلاسيكية لا تتعدى 188 بايت، وأن مفاتيح Kyber-512 العامة والنصوص المشفرة الخاصة بها معقولة الحجم، فإن كلا رسالتي بدء الاتصال تتناسبان داخل وحدة التحكم القصوى القياسية (MTU) لبروتوكول UDP. ويتم استخدام المفتاح المشترك الناتج (osk) من عملية بدء الاتصال PQ KK كمفتاح مشترك مسبقًا (psk) مدخل لعملية بدء اتصال Wireguard IK القياسية. وهكذا تُجرى عمليتا بدء اتصال كاملتان، إحداهما نقية باستخدام التشفير ما بعد الكمي (PQ) والأخرى نقية باستخدام X25519.

لا يمكننا فعل أي من هذا لاستبدال عمليات الاتصال XK وIK لأن:
...
توجد الكثير من المعلومات القيّمة في الورقة البيضاء، وسنقوم بمراجعتها للحصول على الأفكار والإلهام. المطلوب: إنجاز.

## المواصفة

### الهياكل الشائعة

حدّث الأقسام والجداول في وثيقة الهياكل المشتركة [/docs/specs/common-structures/](/docs/specs/common-structures/) على النحو التالي:
...
مفاتيح العموم الهجينة هي مفتاح X25519. أما مفاتيح العموم الخاصة بـ KEM فهي المفتاح العابر المتعلق بالحوسبة الكمية (PQ) الذي تُرسله أليس إلى بوب. ويُعرَّف الترميز وترتيب البايتات في [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf).

مفاتيح MLKEM*_CT ليست مفاتيح عامة بالمعنى الحقيقي، بل هي "نص مشفر" يُرسله بوب إلى أليس أثناء عملية التحاور في Noise. تم سردها هنا لأغراض اكتمال المعلومات.

### المفتاح الخاص

أنواع المفتاح الخاص الجديد هي:
...
المفاتيح الخاصة الهجينة هي مفاتيح X25519. أما المفاتيح الخاصة لتقنية KEM فهي مخصصة لأليس فقط. ويُعرّف ترميز KEM وترتيب البايتات في [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf).
...
نموذج التهديد الأكثر إثارة للقلق في الحوسبة الكمية (PQ) الآن هو تخزين حركة البيانات اليوم، لفك تشفيرها بعد سنوات عديدة من الآن (السرية المستقبلية). النهج المختلط سيوفر الحماية من ذلك.
...
إذن، أقدم نموذج تهديد PQ هو قيام OBEP/IBGW بتخزين حركة المرور لفك تشفيرها لاحقاً. يجب أن نطبق hybrid ratchet أولاً.
