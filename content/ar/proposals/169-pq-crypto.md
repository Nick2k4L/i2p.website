...
التغييرات: يحتوي NTCP2 الحالي فقط على الخيارات في قسم تشاتشا واحد. مع ML-KEM، سيكون هناك قسم تشاتشا جديد قبل الخيارات، يحتوي على نص التشفير الكمي المشفر (PQ ciphertext).
...
استخدم نفس العنوان/المنفذ كما في الاتصال غير المقاوم للحاسوب الكمي وغير المحجوب بجدار الحماية. يتم دعم متغير واحد فقط مقاوم للحاسوب الكمي. في عنوان router، انشر v=2 (كالمعتاد) والمعامل الجديد pq=[3|4|5] للإشارة إلى MLKEM 512/768/1024. تقوم Alice بتعيين البت الأكثر أهمية للمفتاح المؤقت (key[31] & 0x80) في طلب الجلسة للإشارة إلى أن هذا اتصال هجين. انظر أعلاه. ستتجاهل أجهزة router الأقدم معامل pq وتتصل بشكل غير مقاوم للحاسوب الكمي كالمعتاد.
...
التغييرات: يحتوي SSU2 الحالي فقط على بيانات الكتلة في قسم ChaCha واحد. ومع ML-KEM، سيكون هناك قسم ChaCha جديد قبل بيانات الكتلة، يحتوي على المفتاح العمومي للتشفير الكمي (PQ) المشفر.

تغيير KDF لحماية من التزييف: لمعالجة القضايا المطروحة في الاقتراح 165 [Prop165]_، ولكن بحل مختلف، نقوم بتعديل KDF لطلب الجلسة. هذا التعديل ينطبق فقط على الجلسات PQ. بينما يبقى KDF للجلسات غير PQ دون تغيير.
...
التغييرات: يحتوي SSU2 الحالي فقط على الحمولة في قسم واحد من نوع ChaCha. ومع ML-KEM، سيكون هناك قسم جديد من نوع ChaCha قبل الحمولة، يحتوي على نص مشفر كمي (PQ) مشفر.
...
كـ Alice، لاتصال PQ، قبل التشويش، اضبط X[31] |= 0x80. هذا يجعل X مفتاحًا عامًا غير صالح لـ X25519. بعد التشويش، ستقوم AES-CBC بجعله عشوائيًا. سيكون MSB الخاص بـ X عشوائيًا بعد التشويش.
...
ملاحظة: أكواد الأنواع للاستخدام الداخلي فقط. ستبقى routers من النوع 4، وسيتم الإشارة إلى الدعم في عناوين router.
...
قد تكون هذه موجودة في leaseSet مع مفاتيح X25519 من النوع 4 الأقدم. ستتجاهل أجهزة router الأقدم المفاتيح غير المعروفة.
