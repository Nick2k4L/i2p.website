...
| البروتوكول / الميزة | الحالة |
|--------------------|--------|
| Hybrid MLKEM Ratchet و LS | معتمد 2025-06؛ تجريبي 2025-08؛ إصدار 2025-11 |
| Hybrid MLKEM NTCP2 | تم اختباره على الشبكة المباشرة، معتمد 2026-02؛ هدف تجريبي 2026-05؛ هدف إصدار 2026-08 |
| Hybrid MLKEM SSU2 | معتمد 2026-02؛ هدف تجريبي 2026-08؛ هدف إصدار 2026-11 |
| MLDSA SigTypes 12-14 | الاقتراح مستقر ولكن قد لا يتم الانتهاء منه حتى 2027 |
...
| MLDSA65ph | 19 |
| MLDSA87ph | 20 |
ستستخدم شهادات X.509 والترميزات الأخرى لـ DER الهياكل المركبة وأرقام التعريف (OIDs) المعرّفة في [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/).
...
مفاتيح التوقيع العامة المركبة الهجينة تتكون من مفتاح PQ متبوعًا بمفتاح Ed25519، كما هو موضح في [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/). ويُعرَّف الترميز وترتيب البايتات في [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf).
...
مفاتيح التوقيع الخاصة المركبة الهجينة تتكون من مفتاح PQ يليه مفتاح Ed25519، كما هو موضح في [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/). ويتم تعريف الترميز وترتيب البايتات في [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf).

لن يتم إرسال المفاتيح الخاصة الخاصة بالتوقيع عبر السلك أبدًا. يمكن للتطبيقات اختيار تخزين البذرة ذات 32 بتًا كما هو موصى به في [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)، بدلاً من المفتاح الخاص الموسع الذي يبلغ حجمه عدة كيلوبايت. وهذا يعتمد على التنفيذ.

#### التوقيع

أنواع التوقيعات الجديدة هي:
...
التوقيعات الهجينة المركبة هي توقيعات PQ تليها توقيعات Ed25519، كما هو موضح في [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/). يتم التحقق من التوقيعات الهجينة من خلال التحقق من كلا التوقيعين، ويفشل التحقق إذا فشل أحدهما. ويُعرَّف الترميز وترتيب البايتات في [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf).
...
أنواع المفاتيح العامة التشفيرية الجديدة هي:
...
للمُصادر ذات أنواع التوقيع الهجينة أو PQ، استخدم NONE (النوع 255) لنوع التشفير، ولكن لا يوجد مفتاح تشفير، ويُستخدم القسم الرئيسي بأكمله المكوَّن من 384 بايت لمفتاح التوقيع.
...
بدون مساحة فارغة. الطول الكلي هو 7 + الطول الكلي للمفتاح. طول شهادة المفتاح هو 4 + طول المفتاح الزائد.
...
مثلاً تدفق بايتات هوية جهاز توجيه بحجم 1351 بايت لـ MLDSA44:
...
أضف مواصفة جديدة لخوارزميات التوقيع المركبة، كما يلي: التوقيعات الهجينة المركبة مُعرّفة كما ورد في [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/). ومع ذلك، كما هو معتاد، فإن المفاتيح العامة والتوقيعات داخل I2P تستثني الترميزات DER.

تستخدم التوقيعات المركبة دائمًا عملية ما قبل التجزئة، وبالتالي لا يلزم معالجة الرسائل الكبيرة محتملًا مرتين. هذه العملية خارجية لخوارزمية MLDSA، ونحن نستخدم خوارزمية MLDSA القياسية، وليس HashML-DSA.
...
نفس خوارزمية التوقيع. الفشل إذا فشل أي من التوقيعين.
...
[COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/) لا يُعرّف تركيبة MLDSA87 + Ed25519، على الأرجح بسبب اختلاف قوة الأمان بينهما. لكنه يُعرّف التركيبة MLDSA87 + Ed448، باستخدام دالة التجزئة المسبقة SHAKE256/64. هذه التركيبة غير مدرجة حاليًا في هذا الاقتراح، لأننا لا ندعم Ed448 في الوقت الراهن.

### أنماط المصافحة

تستخدم المصافحات أنماط بروتوكول [Noise](https://noiseprotocol.org/noise.html).
...
تُجرى التعديلات التالية على XK وIK من أجل السرية التقدّمية الهجينة (hfs) كما هو محدّد في القسم 5 من [مواصفة Noise HFS](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf):
...
```
يُعرّف النمط e1 على النحو التالي، كما هو محدد في القسم 4 من [مواصفات ضوضاء HFS](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf):
...
For Alice:
  ...
  MixHash(ciphertext)
```
يُعرّف نمط ekem1 كما يلي، كما هو محدد في القسم 4 من [مواصفات Noise HFS](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf):
...
ينطبق هذا القسم على بروتوكولي IK و XK على حد سواء.

يُعرَّف الاتصال اليدوي الهجين في [مواصفات Noise HFS](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf). الرسالة الأولى، من آليس إلى بوب، تحتوي على e1، وهي مفتاح التغليف، قبل حمولة الرسالة. ويتم التعامل مع هذا المفتاح كمفتاح ثابت إضافي؛ قم باستدعاء دالة EncryptAndHash() عليه (كآليس) أو DecryptAndHash() (كبوب). ثم تتم معالجة حمولة الرسالة كالمعتاد.
...
نحن نعرّف الدوال التالية المقابلة للكتل البنائية التشفيرية المستخدمة كما هو محدد في [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf).
...
(النص_المشفر، المفتاح_المشترك_kem) = ENCAPS(مفتاح_التشفير)
...
لاحظ أن المفتاح المشفر (encap_key) والنص المشفر (ciphertext) يتم تشفيرهما داخِل كُتَل ChaCha/Poly في رسائِل مصافحة Noise 1 و2. وسوف يتم فك تشفيرها كجزء من عملية المصافحة.

يتم خلط kem_shared_key في مفتاح التسلسل باستخدام MixHash(). انظر أدناه للتفاصيل.

#### Alice KDF للرسالة 1

لـ XK: بعد نمط رسالة 'es' وقبل الحمولة، أضف:

أو

للـ IK: بعد نمط الرسالة 'es' وقبل نمط الرسالة 's'، أضف:
...
لـ XK: بعد نمط رسالة "ee" وقبل الحمولة، أضف ما يلي:
...
قم بتحديث مواصفات ECIES-Ratchet [/docs/specs/ecies/](/docs/specs/ecies/) على النحو التالي:
...
التغييرات: الراتش الحالي يحتوي على حمولة فارغة في القسم الأول من تشاتشا، والحمولة في القسم الثاني. مع ML-KEM، أصبح هناك الآن ثلاثة أقسام. القسم الأول يحتوي على النص المشفر الكمي (PQ ciphertext)، القسم الثاني يحتوي على حمولة فارغة، والقسم الثالث يحتوي على الحمولة.
...
التغييرات: يحتوي NTCP2 الحالي فقط على الخيارات في قسم تشاتشا واحد. مع ML-KEM، سيكون هناك قسم تشاتشا جديد قبل الخيارات، يحتوي على المفتاح العمومي للتشفير الكمي (PQ) المشفر.
...
توقيعات PQ: تحتوي كتل Relay وكتل Peer Test ورسائل Peer Test جميعها على توقيعات. للأسف، توقيعات PQ أكبر من MTU. لا توجد آلية حالية لتجزئة كتل Relay أو Peer Test أو الرسائل عبر حزم UDP متعددة. يجب توسيع البروتوكول لدعم التجزئة. سيتم ذلك في اقتراح منفصل سيحدد لاحقاً. حتى اكتمال ذلك، لن يتم دعم Relay و Peer Test.
...
نموذج التهديد PQ لكسر مفاتيح المصادقة في فترة زمنية معقولة (لنقل بضعة أشهر) ثم انتحال هوية المصادقة أو فك التشفير في الوقت شبه الفوري، هو أبعد بكثير؟ وهذا هو الوقت الذي نرغب فيه في الانتقال إلى مفاتيح PQC الثابتة.

تم تعليق العمل على دعم توقيع MLDSA في I2P حتى أواخر عام 2027 أو 2028، انتظارًا لإكمال الهيئات القياسية لاختيار الخوارزميات، وربما تقليل أحجام المفاتيح و/أو التوقيعات، وتعزيز تبني الصناعة لها. انظر [CABFORUM](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/)، و[COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)، و[PLANTS](https://datatracker.ietf.org/wg/plants/about/). أيضًا، سيكون تبني MLDSA في الصناعة معياريًا من خلال IETF ومنتدى CA/Browser وسلطات الشهادات. تحتاج سلطات الشهادات أولًا إلى دعم وحدات الأمان المادية (HSM)، والذي لا يتوفر حاليًا [CA/Browser Forum](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/). نتوقع أن تقود IETF ومنتدى CA/Browser القرارات المتعلقة باختيارات المعلمات المحددة، بما في ذلك ما إذا كانت ستدعم أو تشترط التوقيعات المركبة [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/).
...
| Ratchet افتراضي | أواخر 2026 |
| Signature beta | أواخر 2026 |
| NTCP2 production | أواخر 2026 |
| SSU2 production | أوائل 2027 |
| اختيار أفضل نوع توقيع | أوائل 2027 |
| NTCP2 افتراضي | أوائل 2027 |
| SSU2 افتراضي | منتصف 2027 |
| Signature production | منتصف 2027 |
## الترحيل

Ratchet له الأولوية العليا. وسائل النقل تأتي بعد ذلك. التوقيعات لها أقل أولوية.

سيكون طرح التوقيع أيضاً متأخراً بعام أو أكثر عن طرح التشفير، لأن التوافق العكسي غير ممكن. كما أن اعتماد MLDSA في الصناعة سيتم توحيده من قبل منتدى CA/Browser وسلطات الشهادات. تحتاج سلطات الشهادات أولاً إلى دعم وحدة الأمان الأجهزة (HSM)، والذي غير متوفر حالياً [CA/Browser Forum](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/). نتوقع أن يقود منتدى CA/Browser القرارات حول اختيارات المعايير المحددة، بما في ذلك ما إذا كان سيتم دعم أو طلب التوقيعات المركبة [IETF draft](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/).

## المشاكل

يُتوقع أن تظل خوارزمية SHA256 آمنة لفترة أخرى تتراوح بين 20 إلى 30 عامًا، ولا تهددها الحوسبة الكمية ما بعد الكيوبتات (PQ)، انظر [عرض NIST](https://csrc.nist.gov/csrc/media/Presentations/2022/update-on-post-quantum-encryption-and-cryptographi/Day%202%20-%20230pm%20Chen%20PQC%20ISPAB.pdf) و[عرض NCCOE](https://www.nccoe.nist.gov/sites/default/files/2023-08/pqc-light-at-the-end-of-the-tunnel-presentation.pdf). إذا تم كسر SHA256، فسنواجه مشكلات أخطر بكثير (مثل تلف قاعدة البيانات الشبكية netDb).
