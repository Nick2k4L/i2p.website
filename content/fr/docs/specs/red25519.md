...
description: "Schéma de signature re-randomisable pour créer des Destinations aveugles"
slug: "red25519"
aliases:
  - "/fr/docs/specs/red25519-signature-scheme"
  - "/fr/docs/specs/red25519-signature-scheme/"
category: "Conception"
lastUpdated: "2020-08"
accurateFor: "0.9.47"
...
Ce document spécifie un schéma de signature re-randomisable adapté à la création de Destinations qui peuvent être aveuglées. Il peut également être utilisé pour aveugler des Destinations Ed25519 existantes, avec une légère réduction d'efficacité.

## Motivation

[La proposition 123](/proposals/123-new-netdb-entries) définit un format LeaseSet2 chiffré qui incarne le principe de moindre autorité : chaque participant du réseau ne reçoit que les informations nécessaires à son rôle. En particulier, un LeaseSet2 chiffré publié vers un floodfill ne révèle pas la Destination à laquelle il est destiné, et les Leases ne peuvent être consultés que par quelqu'un ayant une connaissance préalable de la Destination. Cependant, les floodfills doivent encore pouvoir authentifier les LeaseSet2 chiffrés lorsqu'ils sont publiés, et les clients doivent en plus s'assurer que l'authentification a été appliquée par la Destination elle-même.

La Proposition 123 y parvient en aveuglant les clés de signature des Destinations. Les clés aveuglées peuvent être utilisées pour créer des signatures qui sont vérifiables par les floodfills, et les clients peuvent être certains que seule la Destination aurait pu créer les signatures. Il est donc nécessaire de spécifier un schéma de signature qui peut être utilisé pour l'aveuglement.

## Conception

### Schéma de signature principal

Le schéma de signature spécifié ici, Red25519, est une instanciation de [RedDSA](https://github.com/zcash/zips/tree/master/protocol/protocol.pdf), un schéma de signature basé sur Schnorr qui prend en charge la re-randomisation de clés. Il possède les fonctions suivantes :

**GENERATE_PRIVATE()** : Retourne une clé privée aléatoirement uniforme.

**DERIVE_PUBLIC(sk)** : Retourne la clé publique correspondant à la clé privée donnée.

**GENERATE_RANDOM()** : Retourne un scalaire aléatoire adapté pour re-randomiser une paire de clés.
...
Pour une paire de clés donnée (sk, vk), la relation suivante est vraie :
...
Les clés Ed25519 PEUVENT être converties de manière transitoire et unidirectionnelle en clés Red25519, afin de prendre en charge la re-randomisation des Destinations Ed25519 existantes. Les autres sigtypes ne sont pas compatibles.
...
**B** : Le point de base Ed25519 tel que défini dans [RFC-8032](https://tools.ietf.org/html/rfc8032).

**L** : L'ordre Ed25519 2^252 + 27742317777372353535851937790883648493 comme dans [RFC-8032](https://tools.ietf.org/html/rfc8032).

**[s] B** : Multiplication scalaire à base fixe du point de base par s.

**[s] A** : Multiplication scalaire à base variable de A par s.

**x || y** : Concaténer deux tableaux d'octets x et y.

### Red25519

Le schéma Red25519 spécialise RedDSA avec :

- G := le groupe de points sur la forme d'Edwards de Curve25519. En particulier, cela signifie que Red25519 utilise le sous-groupe d'ordre premier d'ordre L, et le cofacteur h_G est 8.
- P_G := le point de base Ed25519 B.
- l_H := 512
- H(x) := SHA-512("I2P_Red25519H(x)" || x)

RedDSA suppose que H(x) est instancié avec une fonction de hachage cryptographique qui est sécurisée contre les attaques d'extension de longueur. SHA-512 ne satisfait pas cela par lui-même. Pour remédier à cela, nous exigeons que les messages soient préfixés avec un encodage sans préfixe de leur longueur :

```
len_u16(M) || M
```
où `len_u16(M)` est la représentation sur 2 octets de la longueur de M, en little-endian (pour être cohérent avec l'encodage little-endian des scalaires et points).
...
Les clés privées Red25519 sont des scalaires mod L, encodées en représentation little-endian. Nous définissons les fonctions `DECODE_SCALAR` et `ENCODE_SCALAR` pour permuter entre les formes de tableau d'octets et d'entier d'un scalaire.

Les clés publiques Red25519 sont des points sur la forme d'Edwards de Curve25519. Elles sont encodées comme la représentation little-endian de 255 bits de la coordonnée y, suivie d'un seul bit indiquant le signe de la coordonnée x. C'est le même encodage que pour Ed25519. Nous définissons les fonctions `DECODE_POINT` et `ENCODE_POINT` pour convertir entre les formes tableau d'octets et coordonnées d'un point.

### Fonctions RedDSA

Pour faciliter l'implémentation, nous détaillons explicitement ci-dessous les fonctions RedDSA (ainsi que plusieurs fonctions auxiliaires) déjà spécialisées pour Red25519. Les implémenteurs doivent se référer à la section 5.4.6 de [RedDSA](https://github.com/zcash/zips/tree/master/protocol/protocol.pdf) pour la spécification générale des fonctions RedDSA.
...
Re-randomiser une Destination Red25519 puis créer des signatures avec celle-ci ne divulgue aucune information sur la Destination, car la distribution des clés privées Red25519 générées via `RANDOMIZE_PRIVATE` est identique à la distribution des clés privées générées via `GENERATE_PRIVATE`, et `DERIVE_PUBLIC` est déterministe.

La conversion des clés privées Ed25519 vers Red25519 via `CONVERT_ED25519_PRIVATE` ne produit pas la même distribution. Cependant, nous considérons la réduction de sécurité acceptable pour les raisons suivantes :

- L'espace des scalaires Ed25519 est approximativement deux fois plus petit que l'espace des scalaires Red25519 (il y a 2^251 scalaires Ed25519 possibles, et L ~= 2^252 scalaires Red25519 possibles). Ainsi la perte de sécurité est au maximum d'un facteur d'environ 2, ou environ 1 bit (parce que nous aurions pu choisir par coïncidence un scalaire Red25519 qui est aussi un scalaire Ed25519 valide).
- Les Destinations Ed25519 existantes ont déjà été historiquement exposées sur le réseau, et il faut supposer que des floodfills malveillants les ont déjà énumérées.

Les utilisateurs préoccupés par cette réduction de sécurité devraient utiliser Red25519 comme sigtype pour leurs Destinations au lieu d'Ed25519.
...
Les versions I2P qui prennent en charge Red25519 pourront vérifier les structures de données réseau qui sont signées avec celui-ci. Les versions I2P qui ne prennent pas en charge Red25519 le traiteront comme une signature inconnue, et PEUVENT abandonner les structures de données. Les utilisateurs doivent s'attendre à ce que la fiabilité des structures de données signées Red25519 soit faible jusqu'à ce qu'un déploiement suffisant de Red25519 soit atteint.
...
- [Proposition 123](/proposals/123-new-netdb-entries)
