---
title: "Red25519 हस्ताक्षर योजना"
description: "blinded Destinations बनाने के लिए पुनः-यादृच्छिकीकरण योग्य हस्ताक्षर योजना"
slug: "red25519"
aliases:
  - "/hi/docs/specs/red25519-signature-scheme"
  - "/hi/docs/specs/red25519-signature-scheme/"
category: "डिज़ाइन"
lastUpdated: "2020-08"
accurateFor: "0.9.47"
...
यह दस्तावेज़ एक पुनः-यादृच्छिकीकरण योग्य हस्ताक्षर योजना निर्दिष्ट करता है जो ऐसे Destinations बनाने के लिए उपयुक्त है जिन्हें blinded किया जा सकता है। यह अतिरिक्त रूप से मौजूदा Ed25519 Destinations को blind करने के लिए उपयोग किया जा सकता है, जिसमें प्रभावशीलता में थोड़ी कमी होती है।

## प्रेरणा

[प्रस्ताव 123](/proposals/123-new-netdb-entries) एक एन्क्रिप्टेड LeaseSet2 प्रारूप को परिभाषित करता है जो न्यूनतम प्राधिकार के सिद्धांत को मूर्त रूप देता है: प्रत्येक नेटवर्क भागीदार को केवल उनकी भूमिका के लिए आवश्यक जानकारी दी जाती है। विशेष रूप से, एक floodfill में प्रकाशित एक एन्क्रिप्टेड LeaseSet2 उस Destination को प्रकट नहीं करता जिसके लिए यह है, और Leases केवल उसी के द्वारा देखे जा सकते हैं जिसके पास Destination का पूर्व ज्ञान है। हालांकि, floodfills को अभी भी एन्क्रिप्टेड LeaseSet2s को प्रमाणित करने में सक्षम होना चाहिए जब वे प्रकाशित किए जाते हैं, और clients को अतिरिक्त रूप से यह सुनिश्चित करना होता है कि प्रमाणीकरण स्वयं Destination द्वारा लागू किया गया था।

प्रस्ताव 123 इसे Destinations की signing keys को blinding करके प्राप्त करता है। blinded keys का उपयोग ऐसे signatures बनाने के लिए किया जा सकता है जो floodfills द्वारा सत्यापित हो सकते हैं, और clients निश्चित हो सकते हैं कि केवल Destination ही उन signatures को बना सकता था। इसलिए एक signature scheme निर्दिष्ट करना आवश्यक है जिसका उपयोग blinding के लिए किया जा सकता है।

## डिज़ाइन

### मूल हस्ताक्षर योजना

यहाँ निर्दिष्ट signature scheme, Red25519, [RedDSA](https://github.com/zcash/zips/tree/master/protocol/protocol.pdf) का एक instantiation है, जो एक Schnorr-आधारित signature scheme है जो key re-randomization का समर्थन करती है। इसमें निम्नलिखित functions हैं:

**GENERATE_PRIVATE()** : एक समान रूप से यादृच्छिक private key वापस करता है।

**DERIVE_PUBLIC(sk)** : दिए गए private key के अनुरूप public key वापस करता है।

**GENERATE_RANDOM()** : एक यादृच्छिक स्केलर वापस करता है जो keypair को पुनः-यादृच्छिक बनाने के लिए उपयुक्त है।

**RANDOMIZE_PRIVATE(sk, alpha)** : एक गुप्त स्केलर alpha का उपयोग करके एक प्राइवेट की को पुनः यादृच्छिक बनाता है।

**RANDOMIZE_PUBLIC(vk, alpha)** : एक गुप्त स्केलर अल्फा का उपयोग करके एक सार्वजनिक कुंजी को पुनः-यादृच्छिक करता है।

**SIGN(sk, m)** : दिए गए संदेश m पर निजी कुंजी sk द्वारा एक हस्ताक्षर लौटाता है।

**VERIFY(vk, m, sig)** : public key vk और message m के विरुद्ध signature sig को सत्यापित करता है। यदि signature वैध है तो true रिटर्न करता है, अन्यथा false।

किसी दिए गए keypair (sk, vk) के लिए निम्नलिखित संबंध होता है:

```
RANDOMIZE_PUBLIC(vk, alpha) == DERIVE_PUBLIC(RANDOMIZE_PRIVATE(sk, alpha))
```
### Ed25519 keys को Red25519 में परिवर्तित करना

Ed25519 keys को अस्थायी रूप से एक-तरफा Red25519 keys में परिवर्तित किया जा सकता है, मौजूदा Ed25519 Destinations के re-randomization को समर्थन देने के लिए। अन्य sigtypes संगत नहीं हैं।

हम निम्नलिखित रूपांतरण फंक्शन को परिभाषित करते हैं:

**CONVERT_ED25519_PRIVATE(privkey)** : दिए गए Ed25519 private key के अनुरूप Red25519 private key को वापस करता है।

**CONVERT_ED25519_PUBLIC(pubkey)** : दिए गए Ed25519 पब्लिक की के अनुरूप Red25519 पब्लिक की रिटर्न करता है।

दिए गए Ed25519 keypair (privkey, pubkey) के लिए निम्नलिखित संबंध होता है:
...
**B** : Ed25519 basepoint जैसा कि [RFC-8032](https://tools.ietf.org/html/rfc8032) में है।

**L** : Ed25519 order 2^252 + 27742317777372353535851937790883648493 जैसा कि [RFC-8032](https://tools.ietf.org/html/rfc8032) में है।

**[s] B** : s द्वारा basepoint का Fixed-base scalar multiplication।

**[s] A** : s द्वारा A का चर-आधार अदिश गुणन।

**x || y** : दो byte arrays x और y को जोड़ना।

### Red25519

Red25519 स्कीम RedDSA को निम्नलिखित के साथ विशेषीकृत करती है:

- G := Curve25519 के Edwards form पर बिंदुओं का समूह। विशेष रूप से, इसका मतलब है कि Red25519 order L के prime-order subgroup का उपयोग करता है, और cofactor h_G 8 है।
- P_G := Ed25519 basepoint B।
- l_H := 512
- H(x) := SHA-512("I2P_Red25519H(x)" || x)

RedDSA मानता है कि H(x) को एक क्रिप्टोग्राफिक hash function के साथ instantiate किया गया है जो length extension attacks के विरुद्ध सुरक्षित है। SHA-512 अपने आप में इसे संतुष्ट नहीं करता। इसे ठीक करने के लिए, हमारी आवश्यकता है कि messages को उनकी length की prefix-free encoding के साथ prefixed किया जाए:

```
len_u16(M) || M
```
जहाँ `len_u16(M)` M की लंबाई का 2-बाइट प्रतिनिधित्व है, little-endian में (scalars और points की little-endian encoding के साथ संगत रहने के लिए)।

संदेश 65534 बाइट्स से अधिक लंबे नहीं होने चाहिए। 65535 की लंबाई संभावित भविष्य के विस्तार के लिए आरक्षित है।

### एन्कोडिंग और डिकोडिंग

Red25519 private keys scalars mod L हैं, जो little-endian representation में encoded हैं। हम `DECODE_SCALAR` और `ENCODE_SCALAR` functions को define करते हैं scalar के byte array और integer forms के बीच swap करने के लिए।

Red25519 public keys Edwards form की Curve25519 पर points होती हैं। ये y-coordinate के 255-bit little-endian representation के रूप में encode होती हैं, जिसके बाद x-coordinate के sign को दर्शाने वाला एक single bit होता है। यह encoding Ed25519 के समान है। हम byte array और point के coordinate forms के बीच swap करने के लिए `DECODE_POINT` और `ENCODE_POINT` functions को define करते हैं।

### RedDSA फ़ंक्शन

कार्यान्वयन की सुविधा के लिए, हम नीचे स्पष्ट रूप से RedDSA functions (साथ ही कई helper functions) लिख रहे हैं जो पहले से ही Red25519 के लिए विशेषीकृत हैं। कार्यान्वयनकर्ताओं को RedDSA functions के सामान्य विनिर्देश के लिए [RedDSA](https://github.com/zcash/zips/tree/master/protocol/protocol.pdf) के खंड 5.4.6 का संदर्भ लेना चाहिए।
...
```
ध्यान दें कि `CONVERT_ED25519_PRIVATE` का implementation Ed25519 private key से Ed25519 public key derive करते समय secret scalar s की computation के बराबर है, जैसा कि [RFC-8032](https://tools.ietf.org/html/rfc8032) के section 5.1.5 के steps 1-3 में निर्दिष्ट है।

## सुरक्षा प्रभाव

Red25519 Destination को फिर से randomize करना और फिर उसके साथ signatures बनाना Destination के बारे में कोई जानकारी leak नहीं करता, क्योंकि `RANDOMIZE_PRIVATE` के माध्यम से generate किए गए Red25519 private keys का distribution `GENERATE_PRIVATE` के माध्यम से generate किए गए private keys के distribution के समान है, और `DERIVE_PUBLIC` deterministic है।

Ed25519 private keys को `CONVERT_ED25519_PRIVATE` के माध्यम से Red25519 में परिवर्तित करने से समान वितरण नहीं मिलता है। हालांकि, हम निम्नलिखित कारणों से security में कमी को स्वीकार्य मानते हैं:

- Ed25519 scalars का स्थान Red25519 scalars के स्थान से लगभग आधा है (2^251 संभावित Ed25519 scalars हैं, और L ~= 2^252 संभावित Red25519 scalars हैं)। इस प्रकार सुरक्षा की हानि अधिकतम लगभग 2 के कारक के बराबर है, या लगभग 1 bit (क्योंकि हम संयोग से एक Red25519 scalar चुन सकते थे जो एक वैध Ed25519 scalar भी है)।
- मौजूदा Ed25519 Destinations पहले से ही नेटवर्क पर ऐतिहासिक रूप से उजागर हो चुके हैं, और यह मान लेना चाहिए कि दुर्भावनापूर्ण floodfills ने पहले ही उनकी गणना कर ली है।

जो उपयोगकर्ता इस सुरक्षा कमी को लेकर चिंतित हैं, उन्हें अपने Destinations के लिए Ed25519 के बजाय Red25519 को sigtype के रूप में उपयोग करना चाहिए।

ध्यान दें कि उपरोक्त तर्क re-randomization scalar alpha पर लागू नहीं होता है; key के बारे में जानकारी हर बार एक biased alpha चुने जाने पर लीक हो जाती है, क्योंकि additive re-randomization एक one-time pad की तरह व्यवहार करता है।

## संगतता

I2P versions जो Red25519 को support करते हैं वे network datastructures को verify कर सकेंगे जो इससे signed हैं। I2P versions जो Red25519 को support नहीं करते वे इसे एक unknown signature के रूप में treat करेंगे, और datastructures को drop कर सकते हैं। Users को expect करना चाहिए कि Red25519-signed datastructures की reliability तब तक poor रहेगी जब तक Red25519 का sufficient deployment नहीं हो जाता।
