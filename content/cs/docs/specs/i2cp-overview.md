---
title: "Přehled I2CP"
description: "Přehled I2P klientského protokolu (I2CP) – správa relací, možnosti, formát užitečného zatížení a multiplexování."
slug: "i2cp-overview"
aliases: 
category: "Protokoly"
lastUpdated: "2025-04"
...
Některé implementace směrovače a klienta mohou podporovat také externí připojení přes SSL, jak je nakonfigurováno volbou `i2cp.SSL=true`. I když SSL není výchozí, doporučuje se jej používat pro veškerý provoz, který může být vystaven otevřenému Internetu. Uživatelské jméno a heslo pro autorizaci (pokud je použito), [soukromý klíč](/docs/specs/common-structures/#type_PrivateKey) a [klíč pro podepisování](/docs/specs/common-structures/#type_SigningPrivateKey) pro [cíl](/docs/specs/common-structures/#struct_Destination) jsou všechny přenášeny v otevřené podobě, pokud není SSL povoleno. Některé implementace směrovače a klienta mohou podporovat také externí připojení přes doménové sokety.

## Specifikace protokolu I2CP

Kompletní specifikaci protokolu najdete na stránce [specifikace I2CP](/docs/specs/i2cp/).

## Inicializace I2CP {#initialization}

Když se klient připojí k routeru, nejprve pošle jeden bajt určující verzi protokolu (0x2A). Poté pošle [Zprávu GetDate](/docs/specs/i2cp/#msg_GetDate) a čeká na odpověď [Zprávy SetDate](/docs/specs/i2cp/#msg_SetDate). Následně pošle [Zprávu CreateSession](/docs/specs/i2cp/#msg_CreateSession) obsahující konfiguraci relace. Poté čeká na [Zprávu RequestLeaseSet](/docs/specs/i2cp/#msg_RequestLeaseSet) od routeru, která signalizuje, že byly vytvořeny příchozí tunely, a odpoví zprávou CreateLeaseSetMessage obsahující podepsaný LeaseSet. Klient nyní může iniciovat nebo přijímat spojení od jiných cílů v síti I2P.

## Možnosti I2CP {#options}

### Možnosti na straně směrovače

Následující možnosti jsou tradičně předávány routeru prostřednictvím [SessionConfig](/docs/specs/i2cp/#struct_SessionConfig) obsažené v zprávě [CreateSession Message](/docs/specs/i2cp/#msg_CreateSession) nebo v zprávě [ReconfigureSession Message](/docs/specs/i2cp/#msg_ReconfigureSession).
...
Poznámka: Všechny argumenty, včetně čísel, jsou řetězce. Hodnoty true/false jsou řetězce nezávislé na velikosti písmen. Cokoli jiného než „true“ (bez rozlišení velikosti písmen) je interpretováno jako false. Názvy všech možností rozlišují velikost písmen.

## Formát datové části I2CP a multiplexování {#format}

End-to-end zprávy zpracovávané přes I2CP (tj. data odeslaná klientem ve zprávě [SendMessageMessage](/docs/specs/i2cp/#msg_SendMessage) a přijatá klientem ve zprávě [MessagePayloadMessage](/docs/specs/i2cp/#msg_MessagePayload)) jsou komprimovány pomocí standardního 10bajtového gzip hlavičky začínající sekvencí 0x1F 0x8B 0x08, jak je specifikováno v [RFC 1952](http://www.ietf.org/rfc/rfc1952.txt). Počínaje verzí 0.7.1 I2P využívá nepoužívané části gzip hlavičky k vložení informací o protokolu, zdrojovém portu (from-port) a cílovém portu (to-port), čímž umožňuje použití streamování a datagramů na stejné destinaci a spolehlivé fungování dotazů/odpovědí pomocí datagramů i při více kanálech.
...
Poznámka: Čísla protokolů I2P 224–254 jsou vyhrazena pro experimentální protokoly. Číslo protokolu I2P 255 je vyhrazeno pro budoucí rozšíření.

Integrita dat je ověřena standardní kontrolní sumou gzip CRC-32 dle [RFC 1952](http://www.ietf.org/rfc/rfc1952.txt).

## Důležité rozdíly oproti standardnímu IP {#ip-differences}
