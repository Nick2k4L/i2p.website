...
| MLDSA65ph | 19 |
| MLDSA87ph | 20 |
Chứng chỉ X.509 và các mã hóa DER khác sẽ sử dụng các cấu trúc ghép và OID được định nghĩa trong [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/).
...
Khóa công khai ký lai tổ hợp gồm khóa PQ theo sau là khóa Ed25519, như trong [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/). Việc mã hóa và thứ tự byte được định nghĩa trong [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf).
...
Khóa riêng để ký hỗn hợp tổ hợp bao gồm khóa PQ theo sau là khóa Ed25519, như trong [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/). Việc mã hóa và thứ tự byte được định nghĩa trong [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf).

Các khóa riêng dùng để ký không bao giờ được truyền qua mạng. Các ứng dụng có thể chọn lưu trữ seed 32 bit như khuyến nghị trong [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/) thay vì lưu khóa riêng đã được mở rộng với kích thước hàng KB. Điều này phụ thuộc vào cách triển khai cụ thể.
...
Chữ ký lai tổng hợp là chữ ký PQ theo sau bởi chữ ký Ed25519, như trong [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/). Chữ ký lai được xác minh bằng cách xác minh cả hai chữ ký, và thất bại nếu một trong hai xác minh thất bại. Việc mã hóa và thứ tự byte được định nghĩa trong [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf).
...
Không có độ đệm. Tổng độ dài là 7 cộng với tổng độ dài khóa. Độ dài chứng chỉ khóa là 4 cộng với độ dài khóa dư thừa.
...
Luồng byte định danh bộ định tuyến ví dụ 1351 byte cho MLDSA44:
...
Thêm một đặc tả mới cho các thuật toán chữ ký ghép, như sau: Chữ ký lai ghép được định nghĩa trong [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/). Tuy nhiên, như thông lệ, các khóa công khai và chữ ký trong I2P bỏ qua mã hóa DER.

Chữ ký ghép luôn sử dụng băm trước (prehashing) nên các thông điệp có thể rất lớn sẽ không cần được xử lý hai lần. Điều này nằm ngoài thuật toán MLDSA; chúng tôi sử dụng MLDSA tiêu chuẩn, không phải HashML-DSA.
...
Giống như thuật toán ký. Thất bại nếu một trong hai chữ ký không thành công.
...
[COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/) không định nghĩa tổ hợp MLDSA87 + Ed25519, có lẽ do sự không tương xứng về độ mạnh bảo mật. Tài liệu này định nghĩa tổ hợp MLDSA87 + Ed448, sử dụng SHAKE256/64 làm hàm băm tiền xử lý. Tổ hợp này hiện không được đưa vào đề xuất này, vì chúng tôi hiện chưa hỗ trợ Ed448.
...
Các sửa đổi sau đây đối với XK và IK cho tính bảo mật chuyển tiếp lai (hfs) được quy định như trong mục 5 của [Thông số kỹ thuật HFS của Noise](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf):
...
```
Mẫu e1 được định nghĩa như sau, như đã nêu trong mục 4 của [đặc tả Noise HFS](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf):
...
For Alice:
  ...
  MixHash(ciphertext)
```
Mẫu ekem1 được định nghĩa như sau, như đã nêu trong mục 4 của [Thông số kỹ thuật Noise HFS](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf):
...
Lớp bắt tay lai được định nghĩa trong [Noise HFS spec](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf). Thông điệp đầu tiên, từ Alice đến Bob, chứa e1, khóa đóng gói, trước phần tải của thông điệp. Giá trị này được xử lý như một khóa tĩnh bổ sung; gọi hàm EncryptAndHash() với nó (theo vai Alice) hoặc DecryptAndHash() (theo vai Bob). Sau đó xử lý phần tải của thông điệp như bình thường.
...
Thay đổi: Ratchet hiện tại có phần tải rỗng ở phần đầu tiên ChaCha, và phần tải ở phần thứ hai. Với ML-KEM, giờ đây có ba phần. Phần đầu tiên chứa văn bản mã hóa PQ đã được mã hóa. Phần thứ hai có phần tải rỗng. Phần thứ ba chứa phần tải.
...
Đối với Alice, sau khi tin nhắn được mã hóa bởi Noise, nhưng trước khi che giấu bằng AES đối với X, đặt X[31] |= 0x7f.
...
Trong các tin nhắn tiếp theo, hãy đặt trường ver (phiên bản) trong tiêu đề dài thành 2, như thông thường, ngay cả khi hỗ trợ MLKEM-512 hoặc MLKEM-768. Các triển khai cũng có thể đặt giá trị thành 3 hoặc 4 nếu đầu bên kia hỗ trợ, nhưng điều này không bắt buộc. Các triển khai nên chấp nhận bất kỳ giá trị nào từ 2 đến 4.

- (7) Kiểm tra ngang hàng (tin nhắn ngoài phiên 5-7)

Thảo luận: Việc thiết lập trường phiên bản thành 3 hoặc 4 có thể không bắt buộc đối với tất cả các loại tin nhắn, nhưng việc này hỗ trợ phát hiện lỗi sớm hơn đối với các kết nối hậu lượng tử không được hỗ trợ. Các tin nhắn Yêu cầu Token và Thử lại (loại 9 và 10) nên có phiên bản 3/4 để đảm bảo tính nhất quán. Các tin nhắn Hole Punch (loại 11) có thể không cần áp dụng cách này, nhưng chúng ta sẽ tuân theo cùng một mẫu để đảm bảo tính đồng nhất. Tin nhắn Kiểm thử ngang hàng (loại 7) nằm ngoài phiên và không thể hiện ý định khởi tạo phiên.
...
- Đặt tên / sổ địa chỉ / máy chủ chuyển tiếp
- Tài liệu khác
...
- Nếu thông điệp 1 lớn hơn hoặc bằng 919 byte, có khả năng là MLKEM512_X25519.
