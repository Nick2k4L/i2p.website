...
description: "Схема перерандомизируемых подписей для создания скрытых Destination"
slug: "red25519"
aliases:
  - "/ru/docs/specs/red25519-signature-scheme"
  - "/ru/docs/specs/red25519-signature-scheme/"
category: "Дизайн"
lastUpdated: "2020-08"
accurateFor: "0.9.47"
...
Данный документ определяет схему повторно рандомизируемых подписей, подходящую для создания Destination (точек назначения), которые могут быть скрыты. Дополнительно она может использоваться для сокрытия существующих Ed25519 Destination с небольшим снижением эффективности.

## Мотивация

[Предложение 123](/proposals/123-new-netdb-entries) определяет зашифрованный формат LeaseSet2, который воплощает Принцип Наименьших Полномочий: каждому участнику сети предоставляется только информация, необходимая для его роли. В частности, зашифрованный LeaseSet2, опубликованный на floodfill, не раскрывает Destination, для которого он предназначен, а Leases могут просматривать только те, кто заранее знает Destination. Однако floodfill-узлы по-прежнему должны иметь возможность аутентифицировать зашифрованные LeaseSet2 при их публикации, а клиенты дополнительно должны убеждаться, что аутентификация была обеспечена самим Destination.

Предложение 123 достигает этого путем ослепления ключей подписи Destinations. Ослепленные ключи могут использоваться для создания подписей, которые можно проверить с помощью floodfill, и клиенты могут быть уверены, что только Destination мог создать эти подписи. Поэтому необходимо указать схему подписи, которая может использоваться для ослепления.

## Дизайн

### Основная схема подписи

Схема подписи, описанная здесь, Red25519, является реализацией [RedDSA](https://github.com/zcash/zips/tree/master/protocol/protocol.pdf), схемы подписи на основе Schnorr, которая поддерживает повторную рандомизацию ключей. Она имеет следующие функции:

**GENERATE_PRIVATE()** : Возвращает равномерно-случайный приватный ключ.

**DERIVE_PUBLIC(sk)** : Возвращает публичный ключ, соответствующий данному приватному ключу.

**GENERATE_RANDOM()** : Возвращает случайный скаляр, подходящий для повторной рандомизации пары ключей.

**RANDOMIZE_PRIVATE(sk, alpha)** : Повторно рандомизирует приватный ключ, используя секретный скаляр alpha.

**RANDOMIZE_PUBLIC(vk, alpha)** : Перерандомизирует публичный ключ, используя секретный скаляр alpha.

**SIGN(sk, m)** : Возвращает подпись приватным ключом sk для данного сообщения m.

**VERIFY(vk, m, sig)** : Проверяет подпись sig против открытого ключа vk и сообщения m. Возвращает true, если подпись действительна, иначе false.
...
Ключи Ed25519 МОГУТ быть временно преобразованы в односторонннем порядке в ключи Red25519 для поддержки перерандомизации существующих Destination с Ed25519. Другие типы подписей несовместимы.

Мы определяем следующие функции преобразования:

**CONVERT_ED25519_PRIVATE(privkey)** : Возвращает приватный ключ Red25519, соответствующий данному приватному ключу Ed25519.

**CONVERT_ED25519_PUBLIC(pubkey)** : Возвращает открытый ключ Red25519, соответствующий данному открытому ключу Ed25519.
...
**B** : Базовая точка Ed25519, как в [RFC-8032](https://tools.ietf.org/html/rfc8032).

**L** : Порядок Ed25519 2^252 + 27742317777372353535851937790883648493 как в [RFC-8032](https://tools.ietf.org/html/rfc8032).

**[s] B** : Умножение базовой точки на скаляр s с фиксированным основанием.

**[s] A** : Скалярное умножение A на s с переменным основанием.

**x || y** : Объединить два массива байтов x и y.

### Red25519

Схема Red25519 специализирует RedDSA с:

- G := группа точек на форме Эдвардса кривой Curve25519. В частности, это означает, что Red25519 использует подгруппу простого порядка L, и кофактор h_G равен 8.
- P_G := базовая точка Ed25519 B.
- l_H := 512
- H(x) := SHA-512("I2P_Red25519H(x)" || x)

RedDSA предполагает, что H(x) реализован с помощью криптографической хеш-функции, которая защищена от атак расширения длины. SHA-512 сама по себе не удовлетворяет этому требованию. Чтобы исправить это, мы требуем, чтобы сообщения начинались с префиксного кодирования их длины без префиксов:

```
len_u16(M) || M
```
где `len_u16(M)` — это 2-байтовое представление длины M в формате little-endian (для согласованности с кодированием скаляров и точек в little-endian).

Сообщения не должны превышать 65534 байта. Длина 65535 зарезервирована для возможных будущих расширений.

### Кодирование и декодирование

Приватные ключи Red25519 являются скалярами по модулю L, закодированными в little-endian представлении. Мы определяем функции `DECODE_SCALAR` и `ENCODE_SCALAR` для преобразования между формами скаляра в виде массива байтов и целого числа.

Публичные ключи Red25519 являются точками на форме Эдвардса кривой Curve25519. Они кодируются как 255-битное представление y-координаты в формате little-endian, за которым следует один бит, указывающий знак x-координаты. Это та же кодировка, что используется для Ed25519. Мы определяем функции `DECODE_POINT` и `ENCODE_POINT` для преобразования между формами представления точки в виде массива байтов и координат.

### Функции RedDSA

Для упрощения реализации мы явно приводим ниже функции RedDSA (а также несколько вспомогательных функций), уже специализированные для Red25519. Разработчикам следует обращаться к разделу 5.4.6 [RedDSA](https://github.com/zcash/zips/tree/master/protocol/protocol.pdf) для общей спецификации функций RedDSA.
...
```
Обратите внимание, что реализация `CONVERT_ED25519_PRIVATE` эквивалентна вычислению секретного скаляра s при выводе Ed25519 открытого ключа из Ed25519 закрытого ключа, как указано в шагах 1-3 из раздела 5.1.5 [RFC-8032](https://tools.ietf.org/html/rfc8032).

## Вопросы безопасности

Повторная рандомизация Red25519 Destination и последующее создание подписей с её помощью не раскрывает никакой информации о Destination, поскольку распределение приватных ключей Red25519, генерируемых через `RANDOMIZE_PRIVATE`, идентично распределению приватных ключей, генерируемых через `GENERATE_PRIVATE`, а `DERIVE_PUBLIC` является детерминистическим.

Преобразование приватных ключей Ed25519 в Red25519 через `CONVERT_ED25519_PRIVATE` не приводит к той же самой дистрибуции. Однако мы считаем снижение безопасности приемлемым по следующим причинам:

- Пространство скаляров Ed25519 примерно в два раза меньше пространства скаляров Red25519 (существует 2^251 возможных скаляров Ed25519 и L ~= 2^252 возможных скаляров Red25519). Таким образом, потеря безопасности составляет максимум примерно в 2 раза, или примерно 1 бит (поскольку мы могли случайно выбрать скаляр Red25519, который также является действительным скаляром Ed25519).
- Существующие Ed25519 Destinations уже исторически были раскрыты в сети, и следует предполагать, что вредоносные floodfill уже перечислили их.

Пользователи, обеспокоенные этим снижением безопасности, должны использовать Red25519 в качестве типа подписи для своих назначений (Destinations) вместо Ed25519.

Обратите внимание, что приведенный выше аргумент не применим к скаляру повторной рандомизации alpha; информация о ключе утекает каждый раз при выборе смещенного alpha, поскольку аддитивная повторная рандомизация ведет себя как одноразовый блокнот.

## Совместимость

Версии I2P, которые поддерживают Red25519, смогут верифицировать сетевые структуры данных, подписанные с его помощью. Версии I2P, которые не поддерживают Red25519, будут рассматривать его как неизвестную подпись и МОГУТ отбрасывать структуры данных. Пользователи должны ожидать низкую надежность структур данных, подписанных Red25519, до достижения достаточного развертывания Red25519.
