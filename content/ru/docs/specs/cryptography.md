...
См. код ElGamal [ElGamalEngine](https://github.com/i2p/i2p.i2p/tree/master/core/java/src/net/i2p/crypto/ElGamalEngine.java).
...
Мы используем AES с 256-битными ключами и 128-битными блоками в режиме CBC. Используемое заполнение указано в IETF [RFC-2313](http://tools.ietf.org/html/rfc2313) (PKCS#5 1.5, раздел 8.1 (для типа блока 02)). В данном случае заполнение состоит из псевдослучайно сгенерированных октетов для соответствия 16-байтным блокам. В частности, смотрите код CBC [CryptixAESEngine](https://github.com/i2p/i2p.i2p/tree/master/core/java/src/net/i2p/crypto/CryptixAESEngine.java) и реализацию Cryptix AES [CryptixRijndael_Algorithm](https://github.com/i2p/i2p.i2p/tree/master/core/java/src/net/i2p/crypto/CryptixRijndael_Algorithm.java), а также заполнение, находящееся в функции ElGamalAESEngine.getPadding [ElGamalAESEngine](https://github.com/i2p/i2p.i2p/tree/master/core/java/src/net/i2p/crypto/ElGamalAESEngine.java).
...
Подписи создаются и проверяются с помощью 1024-битного [DSA](http://en.wikipedia.org/wiki/Digital_Signature_Algorithm) (L=1024, N=160), как реализовано в [DSAEngine](https://github.com/i2p/i2p.i2p/tree/master/core/java/src/net/i2p/crypto/DSAEngine.java). DSA был выбран потому, что он работает значительно быстрее для подписей, чем ElGamal.
