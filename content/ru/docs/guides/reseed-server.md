...
Скромная финансовая поддержка может быть доступна тем, кто запускает серверы ресида. StormyCloud предоставляет ежегодную субсидию до 100 долларов США в качестве частичного возмещения расходов на сервер. Право на получение имеют только операторы, которые успешно запускают серверы ресида в течение нескольких месяцев. За информацией обращайтесь к admin на stormycloud dot org.
...
```
Распространённые проблемы: - Маршрутизатор I2P не запущен или сетевая база данных пуста - Порт 8443 уже используется - Проблемы с правами доступа к каталогу `/home/i2p/.reseed/`

### Информация о маршрутизаторе не предоставляется

Убедитесь, что ваш I2P-маршрутизатор запущен и заполнил свою сетевую базу данных:

```bash
ls -lh /home/i2p/.i2p/netDb/
```
Вы должны увидеть множество файлов `.dat`. Если каталог пуст, подождите, пока ваш I2P-маршрутизатор найдёт пиры.

### Ошибки SSL-сертификата

Проверьте, действительны ли ваши сертификаты:
...
Проверьте: - DNS-записи корректно указывают на ваш сервер - брандмауэр разрешает порты 80 и 443 - Nginx Proxy Manager запущен: `docker ps`
...
Запустив сервер повторной инициализации (reseed), вы предоставляете критически важную инфраструктуру для сети I2P. Спасибо, что способствуете созданию более приватного и децентрализованного интернета!

По вопросам или за помощью обращайтесь к сообществу I2P:  
- **Форум**: [i2pforum.net](https://i2pforum.net)  
- **IRC/Reddit**: #i2p в различных сетях  
