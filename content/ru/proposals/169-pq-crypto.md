...
Используйте тот же адрес/порт, что и для не-PQ, не находящегося за файрволлом. Поддерживается один или оба варианта PQ. В адресе router публикуйте v=2 (как обычно) и новый параметр pq=[3|4|3,4|4,3] для указания MLKEM 512/768/оба. Роутеры с MTU меньше минимального, указанного ниже, не должны публиковать параметр "pq", содержащий "4". Публикуйте 4,3 для указания предпочтения MLKEM-768 или 3,4 для указания предпочтения MLKEM-512. Фактическая версия зависит от инициатора, и предпочтение может быть не учтено. Роутеры с MTU меньше минимального, указанного ниже, не должны подключаться используя MLKEM768. Старые роутеры будут игнорировать параметр pq и подключаться без-pq как обычно.

Различный адрес/порт как не-PQ, или только PQ, не за брандмауэром НЕ поддерживается. Это не будет реализовано до тех пор, пока не-PQ SSU2 не будет отключен, через несколько лет. Когда не-PQ будет отключен, поддерживается один или оба PQ варианта. В адресе router, публикуйте v=[3|4|3,4|4,3] для указания MLKEM 512/768/оба. Старые router будут проверять параметр v и пропускать этот адрес как неподдерживаемый.
...
Соблюдайте осторожность, чтобы не превысить MTU при использовании MLKEM768. Минимальный MTU для MLKEM768_X25519 составляет 1318 для IPv4 и 1338 для IPv6 (при условии минимальной полезной нагрузки в 10 байт с блоком DateTime и блоком Padding или RelayTagRequest). Минимальный MTU для SSU2 в целом составляет 1280, поэтому не все узлы могут использовать MLKEM768. Не публикуйте и не используйте MLKEM768, если фактический MTU меньше минимального, либо локально, либо как заявлено узлом. Следите за тем, чтобы размер заполнения не был таким, что сообщение 1 или 2 превысило бы локальный или удаленный MTU.
