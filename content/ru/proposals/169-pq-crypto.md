...
|--------------------|--------|
| Ratchet | Завершён в Java I2P и i2pd |
| NTCP2 | Бета-версия Q1 2026, релиз Q2 2026 |
| SSU2 | Реализация в процессе, бета-версия Q2 2026, релиз Q3 2026 |
| MLDSA SigTypes | Приостановлено до 2027–2028, см. [PLANTS](https://datatracker.ietf.org/wg/plants/about/) |
## Обзор
...
ПРИМЕЧАНИЕ: вся информация в этом предложении, касающаяся подписей MLDSA, является предварительной. Работа над поддержкой подписей MLDSA в I2P приостановлена до конца 2027 или 2028 года, в ожидании результатов работы стандартных организаций по выбору алгоритмов, возможному уменьшению размеров ключей и/или подписей, а также стимулированию их принятия в индустрии. См. [CABFORUM](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/) и [PLANTS](https://datatracker.ietf.org/wg/plants/about/).
...
Таким образом, мы поддержим как исключительно постквантовые, так и гибридные подписи. Мы определим три варианта ML-DSA, как указано в [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf), три гибридных варианта с Ed25519 и три исключительно постквантовых варианта с предварительным хешированием — только для файлов SU3, что в сумме даст 9 новых типов подписей. Гибридные типы будут определены только в комбинации с Ed25519. Мы будем использовать стандартный ML-DSA, а НЕ варианты с предварительным хешированием (HashML-DSA), за исключением файлов SU3.

Мы будем использовать «ограждённый» (hedged) или рандомизированный вариант подписи, а не «детерминированный» вариант, как определено в разделе 3.4 [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf). Это гарантирует, что каждая подпись будет уникальной, даже при подписывании одних и тех же данных, и обеспечивает дополнительную защиту от атак по побочным каналам. Дополнительные сведения о выборе алгоритмов, включая кодирование и контекст, см. в разделе примечаний к реализации ниже.
...
| MLDSA65ph | 19 |
| MLDSA87ph | 20 |
Сертификаты X.509 и другие кодировки DER будут использовать составные структуры и OID, определённые в проекте IETF [IETF draft](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/).

Накладные расходы будут значительными. Типичный размер идентификаторов Ed25519 для узла и маршрутизатора составляет 391 байт. Они увеличатся в 3,5–6,8 раза в зависимости от алгоритма. Подписи Ed25519 — 64 байта — увеличатся в 38–76 раз в зависимости от алгоритма. Типичные подписанные RouterInfo, LeaseSet, ответные датаграммы и сообщения потоковой передачи с подписью составляют около 1 КБ. Они увеличатся в 3–8 раз в зависимости от алгоритма.

Поскольку новые типы идентификаторов назначения и маршрутизатора не будут содержать дополнения, они не будут сжимаемыми. Размеры назначений и идентификаторов маршрутизаторов, сжимаемых в процессе передачи с помощью gzip, увеличатся в 12–38 раз в зависимости от алгоритма.

### Допустимые Комбинации

Для пунктов назначения новые типы подписей поддерживаются со всеми типами шифрования в leaseset. Установите тип шифрования в сертификате ключа как NONE (255).

Для RouterIdentities тип шифрования ElGamal устарел. Новые типы подписей поддерживаются только с шифрованием X25519 (тип 4). Новые типы шифрования будут указаны в RouterAddresses. Тип шифрования в сертификате ключа по-прежнему будет типом 4.
...
Тестовые векторы для SHA3-256, SHAKE128 и SHAKE256 доступны на [NIST](https://csrc.nist.gov/projects/cryptographic-standards-and-guidelines/example-values).

Обратите внимание, что библиотека Java bouncycastle поддерживает всё вышеперечисленное. Поддержка в C++ доступна в OpenSSL 3.5 [OpenSSL](https://openssl-library.org/post/2025-02-04-release-announcement-3.5/).

### Альтернативы

Мы не будем поддерживать [FIPS 205](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.205.pdf) (Sphincs+), так как он намного медленнее и создаёт больший объём данных, чем ML-DSA. Мы не будем поддерживать предстоящий FIPS206 (Falcon), поскольку он ещё не стандартизирован. Мы не будем поддерживать NTRU или другие кандидаты на постквантовую криптографию, которые не были стандартизированы NIST.

### Rosenpass

Существует исследовательская [работа](https://eprint.iacr.org/2020/379.pdf) по адаптации Wireguard (IK) для полностью постквантовой криптографии, однако в ней остаётся несколько нерешённых вопросов. Позднее этот подход был реализован в виде Rosenpass [Rosenpass](https://rosenpass.eu/) [whitepaper](https://raw.githubusercontent.com/rosenpass/rosenpass/papers-pdf/whitepaper.pdf) для постквантового Wireguard.

Rosenpass использует рукопожатие, похожее на Noise KK, с предварительно распределёнными статическими ключами Classic McEliece 460896 (по 500 КБ каждый) и эфемерными ключами Kyber-512 (по сути, MLKEM-512). Поскольку шифртексты Classic McEliece составляют всего 188 байт, а открытые ключи и шифртексты Kyber-512 имеют разумный размер, оба сообщения рукопожатия помещаются в стандартный UDP MTU. Общий выходной ключ (osk) от рукопожатия PQ KK используется в качестве предварительно распределённого ключа (psk) для стандартного рукопожатия Wireguard IK. Таким образом, в общей сложности выполняются два полных рукопожатия: одно полностью постквантовое, а другое — полностью на основе X25519.

Мы не можем сделать это для замены наших рукопожатий XK и IK, потому что:
...
В белой книге содержится множество полезной информации, и мы изучим её в поиске идей и вдохновения. TODO.

## Спецификация

### Общие структуры

Обновите разделы и таблицы в документе по общим структурам [/docs/specs/common-structures/](/docs/specs/common-structures/), как указано ниже:
...
Гибридные открытые ключи — это ключ X25519. Открытые ключи KEM — это временный PQ-ключ, отправляемый от Алисы к Бобу. Кодирование и порядок байтов определены в [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf).

Ключи MLKEM*_CT не являются настоящими открытыми ключами, это «шифротекст», отправляемый от Боба к Элис во время рукопожатия Noise. Они указаны здесь для полноты картины.
...
Гибридные закрытые ключи — это ключи X25519. Закрытые ключи KEM предназначены только для Эллис. Кодирование KEM и порядок байтов определены в [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf).
...
Итак, самая ранняя модель угроз PQ - это OBEP/IBGW, сохраняющие трафик для последующей расшифровки. Мы должны сначала реализовать гибридный ratchet.
