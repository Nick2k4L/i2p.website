...
- Следует ли нам прекратить отправку данных 0-RTT ratchet (кроме LS)?
- Следует ли нам переключить ratchet с IK на XK, если мы не отправляем данные 0-RTT?
...
Изменения: Текущий SSU2 содержит только данные блока в секции ChaCha. С ML-KEM секция ChaCha также будет содержать зашифрованный PQ публичный ключ.
...
TODO: Есть ли более эффективный способ определения подписи/верификации, чтобы избежать копирования подписи?
...
Для PQ-only подписей SU3-файлов используйте OID, определенные в [черновике IETF](https://datatracker.ietf.org/doc/draft-ietf-lamps-dilithium-certificates/) для вариантов без предварительного хеширования в сертификатах. Мы не определяем гибридные подписи SU3-файлов, поскольку нам может потребоваться хешировать файлы дважды (хотя HashML-DSA и X2559 используют одну и ту же хеш-функцию SHA512). Также объединение двух ключей и подписей в сертификате X.509 было бы совершенно нестандартным.

Обратите внимание, что мы запрещаем подписание файлов SU3 с помощью Ed25519, и хотя мы определили подписание Ed25519ph, мы никогда не договорились об OID для него и не использовали его.
...
Обновить другие документы, которые содержат рекомендации по размерам Destination, включая:
...
Типичные размеры ключей, подписей, RIdent, Dest или увеличения размеров (Ed25519 включен для справки), предполагая тип шифрования X25519 для RI. Добавленный размер для Router Info, LeaseSet, датаграмм с возможностью ответа и каждого из двух streaming пакетов (SYN и SYN ACK). Текущие Destinations и Leasesets содержат повторяющееся заполнение и сжимаемы при передаче. Новые типы не содержат заполнения и не будут сжимаемыми, что приведет к гораздо большему увеличению размера при передаче. См. раздел дизайна выше.
...
| MLDSA65 | ??? | ??? |
| MLDSA87 | ??? | ??? |
Предварительные результаты тестов в Java:
...
Категории безопасности NIST обобщены в [презентации NIST](https://www.nccoe.nist.gov/sites/default/files/2023-08/pqc-light-at-the-end-of-the-tunnel-presentation.pdf) на слайде 10. Предварительные критерии: Наша минимальная категория безопасности NIST должна быть 2 для гибридных протоколов и 3 для чисто PQ.
...
Хотя мы определим и реализуем 3 криптографических и 9 типов подписей, мы планируем измерять производительность во время разработки и дополнительно анализировать влияние увеличенных размеров структур. Мы также продолжим исследования и мониторинг разработок в других проектах и протоколах.

После разработки и тестирования мы остановимся на предпочтительном типе или настройке по умолчанию для каждого случая использования. Выбор потребует компромиссов между пропускной способностью, нагрузкой на процессор и предполагаемым уровнем безопасности. Не все типы могут подходить или быть разрешены для всех случаев использования.
...
Библиотеки Bouncycastle, BoringSSL и WolfSSL теперь поддерживают MLKEM и MLDSA. Поддержка OpenSSL будет включена в их релиз 3.5 от 8 апреля 2025 года [OpenSSL](https://openssl-library.org/post/2025-02-04-release-announcement-3.5/).

Библиотека Noise от southernstorm.com, адаптированная для Java I2P, содержала предварительную поддержку гибридных рукопожатий, но мы удалили её как неиспользуемую; нам потребуется добавить её обратно и обновить для соответствия [спецификации Noise HFS](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf).

### Варианты подписи

Мы будем использовать "hedged" или рандомизированный вариант подписи, а не "детерминистический" вариант, как определено в [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf) раздел 3.4. Это гарантирует, что каждая подпись будет отличаться, даже при подписании одних и тех же данных, и обеспечивает дополнительную защиту от атак по побочным каналам. Хотя [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf) указывает, что "hedged" вариант является вариантом по умолчанию, это может быть или не быть правдой в различных библиотеках. Разработчики должны убедиться, что "hedged" вариант используется для подписи.

Мы используем стандартный процесс подписания (называемый Pure ML-DSA Signature Generation), который внутренне кодирует сообщение как 0x00 || len(ctx) || ctx || message, где ctx — это некоторое опциональное значение размером 0x00..0xFF. Мы не используем никакой опциональный контекст. len(ctx) == 0. Этот процесс определен в [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf) Алгоритм 2 шаг 10 и Алгоритм 3 шаг 5. Обратите внимание, что некоторые опубликованные тестовые векторы могут требовать установки режима, в котором сообщение не кодируется.

### Надежность

Увеличение размера приведет к значительно большей фрагментации tunnel для хранилищ NetDB, handshake-операций streaming и других сообщений. Проверьте изменения в производительности и надежности.

### Размеры структур

Найдите и проверьте любой код, который ограничивает размер в байтах информации о router и leaseSet.

### NetDB

Пересмотреть и возможно уменьшить максимальное количество LS/RI, хранящихся в ОЗУ или на диске, чтобы ограничить рост объема хранилища. Увеличить минимальные требования к пропускной способности для floodfill?

### Ratchet

#### Общие туннели

Автоматическая классификация/обнаружение нескольких протоколов в одних и тех же tunnel должна быть возможна на основе проверки длины сообщения 1 (New Session Message). Используя MLKEM512_X25519 в качестве примера, длина сообщения 1 на 816 байт больше, чем у текущего ratchet протокола, а минимальный размер сообщения 1 (с включенной только DateTime полезной нагрузкой) составляет 919 байт. Большинство сообщений 1 с текущим ratchet имеют полезную нагрузку менее 816 байт, поэтому они могут быть классифицированы как не-гибридный ratchet. Большие сообщения, вероятно, являются POST-запросами, которые встречаются редко.
...
Это должно позволить нам эффективно поддерживать стандартный ratchet и гибридный ratchet на одном и том же пункте назначения, точно так же, как мы ранее поддерживали ElGamal и ratchet на одном и том же пункте назначения. Таким образом, мы можем мигрировать к гибридному протоколу MLKEM гораздо быстрее, чем если бы мы не могли поддерживать двойные протоколы для одного и того же пункта назначения, поскольку мы можем добавить поддержку MLKEM к существующим пунктам назначения.
...
Следующие комбинации могут быть сложными и НЕ обязательны для поддержки, но могут поддерживаться в зависимости от реализации:
...
Мы можем не пытаться поддерживать несколько алгоритмов MLKEM (например, MLKEM512_X25519 и MLKEM_768_X25519) на одном и том же назначении. Выберите только один; однако это зависит от того, выберем ли мы предпочтительный вариант MLKEM, чтобы HTTP клиентские tunnel могли его использовать. Зависит от реализации.
...
Мы, вероятно, НЕ будем пытаться поддерживать алгоритмы ElGamal и гибридные алгоритмы на одном и том же destination. ElGamal устарел, а ElGamal + гибридный режим только (без X25519) не имеет особого смысла. Кроме того, сообщения New Session как для ElGamal, так и для гибридного режима имеют большой размер, поэтому стратегии классификации часто должны были бы пробовать оба типа расшифровки, что было бы неэффективно. Зависит от реализации.
...
Спецификация ECIES позволяет включать Garlic Messages в полезную нагрузку New Session Message, что обеспечивает доставку начального потокового пакета за 0-RTT, обычно HTTP GET, вместе с leaseset клиента. Однако полезная нагрузка New Session Message не обладает прямой секретностью. Поскольку данное предложение делает акцент на улучшенной прямой секретности для ratchet, реализации могут или должны отложить включение потоковой полезной нагрузки или полного потокового сообщения до первого Existing Session Message. Это будет за счет доставки за 0-RTT. Стратегии также могут зависеть от типа трафика или типа tunnel, или от GET против POST, например. Зависит от реализации.

#### Размер новой сессии

MLKEM, MLDSA или оба на одном и том же назначении значительно увеличат размер сообщения New Session Message, как описано выше. Это может существенно снизить надежность доставки сообщений New Session Message через туннели, где они должны быть фрагментированы на несколько сообщений туннеля по 1024 байта. Успех доставки пропорционален экспоненциальному количеству фрагментов. Реализации могут использовать различные стратегии для ограничения размера сообщения за счет доставки 0-RTT. Зависит от реализации.

### NTCP2

Мы устанавливаем MSB эфемерного ключа (key[31] & 0x80) в запросе сессии, чтобы указать, что это гибридное соединение. Это позволяет нам запускать как стандартный NTCP, так и гибридный NTCP на одном и том же порту. Поддерживается только один гибридный вариант, который рекламируется в адресе router. Например, v=2,3 или v=2,4 или v=2,5.

#### Обфускация

Как Alice, для PQ соединения, перед обфускацией, установите X[31] |= 0x80. Это делает X недействительным публичным ключом X25519. После обфускации AES-CBC рандомизирует его. MSB X будет случайным после обфускации.

Как Bob, проверьте, что (X[31] & 0x80) != 0 после деобфускации. Если это так, то это PQ соединение.

Минимальная версия router, требуемая для NTCP2-PQ, еще не определена.
...
Проверить и убедиться, что SSU2 может обрабатывать подписанные MLDSA RI, фрагментированные на несколько пакетов (6-8?).

Примечание: Коды типов предназначены только для внутреннего использования. Роутеры останутся типа 4, а поддержка будет указана в адресах роутера.

## Совместимость router

### Названия транспортов

Во всех случаях используйте названия транспортов NTCP2 и SSU2 как обычно.
...
Рекомендуется. Поскольку PQ не влияет на статический ключ X25519 или протоколы handshake N, мы могли бы оставить router как тип 4 и просто анонсировать новые транспорты. Старые router всё ещё могли бы подключаться, строить tunnel через них или отправлять им сообщения netDb.
...
Старые router не могут проверить RI и поэтому не могут подключаться, строить tunnel через них или отправлять сообщения netDb. Потребуется несколько циклов релизов для отладки и обеспечения поддержки перед включением по умолчанию. Это будут те же проблемы, что и при внедрении типов шифрования 5/6/7; может продлить развертывание на год или более по сравнению с альтернативой внедрения типа шифрования 4, указанной выше.
...
Destinations могут поддерживать несколько типов ключей, но только путем пробной расшифровки сообщения 1 каждым ключом. Накладные расходы могут быть снижены за счет ведения подсчета успешных расшифровок для каждого ключа и первоочередной попытки использования наиболее часто используемого ключа. Java I2P использует эту стратегию для ElGamal+X25519 на одном и том же destination.
...
Наиболее ценными данными является end-to-end трафик, зашифрованный с помощью ratchet. Для внешнего наблюдателя между hop-ами tunnel, этот трафик зашифрован дважды дополнительно: tunnel шифрованием и transport шифрованием. Для внешнего наблюдателя между OBEP и IBGW, он зашифрован только один раз дополнительно, transport шифрованием. Для участника OBEP или IBGW, ratchet является единственным шифрованием. Однако, поскольку tunnel-ы являются однонаправленными, перехват обоих сообщений в ratchet handshake потребовал бы сговора router-ов, если только tunnel-ы не были построены с OBEP и IBGW на одном router-е.

Наиболее тревожная модель угроз PQ на данный момент — это сохранение трафика сегодня для расшифровки через много лет (прямая секретность). Гибридный подход защитил бы от этого.

Модель угроз PQ, при которой ключи аутентификации взламываются за разумный период времени (скажем, за несколько месяцев), а затем происходит имитация аутентификации или расшифровка почти в реальном времени, гораздо более отдалена? И именно тогда мы захотим перейти к статическим ключам PQC.

Итак, самая ранняя модель угроз PQ — это OBEP/IBGW, сохраняющие трафик для последующей расшифровки. Сначала нам следует реализовать гибридный ratchet.

Ratchet имеет наивысший приоритет. Транспорты идут следующими. Подписи имеют самый низкий приоритет.

Внедрение подписей также будет отложено на год или более по сравнению с внедрением шифрования, поскольку обратная совместимость невозможна. Кроме того, принятие MLDSA в отрасли будет стандартизировано CA/Browser Forum и центрами сертификации. ЦС сначала нужна поддержка аппаратных модулей безопасности (HSM), которая в настоящее время недоступна [CA/Browser Forum](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/). Мы ожидаем, что CA/Browser Forum будет определять решения по выбору конкретных параметров, включая поддержку или требование составных подписей [IETF draft](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/).
...
Если мы не сможем поддерживать как старые, так и новые ratchet протоколы на одних и тех же tunnel'ах, миграция будет намного сложнее.

Мы должны иметь возможность просто попробовать один, а затем другой, как мы делали с X25519, что должно быть доказано.
