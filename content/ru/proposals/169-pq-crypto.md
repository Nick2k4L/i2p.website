...
До API 0.9.68 (релиз 2.11.0) Java I2P реализовывал максимум 256 байт дополнения для не-PQ соединений, однако это ранее не было документировано. Начиная с API 0.9.69 (релиз 2.12.0), Java I2P реализует такое же максимальное дополнение для не-PQ соединений, как и для MLKEM-512. См. таблицу ниже.

Используйте определенный размер сообщения как максимальное заполнение, то есть максимальное заполнение удвоит размер сообщения для PQ соединений, следующим образом:

| Максимальное дополнение сообщения | non-PQ (до 0.9.68) | non-PQ (начиная с 0.9.69) | MLKEM-512 | MLKEM-768 | MLKEM-1024 |
|-----------------------------------|---------------------|---------------------------|-----------|-----------|------------|
