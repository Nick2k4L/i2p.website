...
description: "Esquema de assinatura re-randomizável para criar Destinations cegados"
slug: "red25519"
aliases:
  - "/pt/docs/specs/red25519-signature-scheme"
  - "/pt/docs/specs/red25519-signature-scheme/"
category: "Design"
lastUpdated: "2020-08"
accurateFor: "0.9.47"
...
Este documento especifica um esquema de assinatura re-randomizável adequado para criar Destinations que podem ser ofuscados. Adicionalmente, pode ser usado para ofuscar Destinations Ed25519 existentes, com uma ligeira redução na eficácia.

## Motivação

[Proposta 123](/proposals/123-new-netdb-entries) define um formato de LeaseSet2 criptografado que incorpora o Princípio da Menor Autoridade: cada participante da rede recebe apenas as informações necessárias para seu papel. Em particular, um LeaseSet2 criptografado publicado em um floodfill não revela o Destination para o qual é destinado, e os Leases só podem ser visualizados por alguém com conhecimento prévio do Destination. No entanto, floodfills ainda precisam ser capazes de autenticar os LeaseSet2s criptografados quando são publicados, e os clientes precisam adicionalmente garantir que a autenticação foi aplicada pelo próprio Destination.

A Proposta 123 consegue isso ofuscando as chaves de assinatura dos Destinations. As chaves ofuscadas podem ser usadas para criar assinaturas que são verificáveis por floodfills, e os clientes podem ter certeza de que apenas o Destination poderia ter criado as assinaturas. Portanto, é necessário especificar um esquema de assinatura que possa ser usado para ofuscação.

## Design

### Esquema de assinatura principal

O esquema de assinatura especificado aqui, Red25519, é uma instanciação do [RedDSA](https://github.com/zcash/zips/tree/master/protocol/protocol.pdf), um esquema de assinatura baseado em Schnorr que suporta re-randomização de chaves. Ele possui as seguintes funções:

**GENERATE_PRIVATE()** : Retorna uma chave privada uniformemente aleatória.
...
**RANDOMIZE_PRIVATE(sk, alpha)** : Re-aleatoriza uma chave privada, usando um escalar secreto alpha.

**RANDOMIZE_PUBLIC(vk, alpha)** : Re-aleatoriza uma chave pública, usando um escalar secreto alpha.

**SIGN(sk, m)** : Retorna uma assinatura pela chave privada sk sobre a mensagem dada m.

**VERIFY(vk, m, sig)** : Verifica a assinatura sig contra a chave pública vk e mensagem m. Retorna verdadeiro se a assinatura for válida, falso caso contrário.

Para um dado par de chaves (sk, vk) a seguinte relação é válida:
...
As chaves Ed25519 PODEM ser convertidas transientemente de forma unidirecional para chaves Red25519, a fim de suportar a re-randomização de Destinations Ed25519 existentes. Outros sigtypes não são compatíveis.

Definimos as seguintes funções de conversão:

**CONVERT_ED25519_PRIVATE(privkey)** : Retorna a chave privada Red25519 correspondente à chave privada Ed25519 fornecida.

**CONVERT_ED25519_PUBLIC(pubkey)** : Retorna a chave pública Red25519 correspondente à chave pública Ed25519 fornecida.

Para um par de chaves Ed25519 dado (privkey, pubkey) a seguinte relação é válida:
...
**B** : O ponto base Ed25519 conforme definido em [RFC-8032](https://tools.ietf.org/html/rfc8032).

**L** : A ordem Ed25519 2^252 + 27742317777372353535851937790883648493 como em [RFC-8032](https://tools.ietf.org/html/rfc8032).
...
- G := o grupo de pontos na forma Edwards da Curve25519. Em particular, isso significa que Red25519 usa o subgrupo de ordem prima de ordem L, e o cofator h_G é 8.
- P_G := o ponto base Ed25519 B.
- l_H := 512
- H(x) := SHA-512("I2P_Red25519H(x)" || x)

RedDSA assume que H(x) é instanciado com uma função hash criptográfica que é segura contra ataques de extensão de comprimento. SHA-512 não satisfaz isso por si só. Para remediar isso, exigimos que as mensagens sejam prefixadas com uma codificação livre de prefixo de seu comprimento:
...
As mensagens não devem ter mais de 65534 bytes. Um comprimento de 65535 é reservado para possíveis extensões futuras.

### Codificação e decodificação

As chaves privadas Red25519 são escalares mod L, codificadas em representação little-endian. Definimos as funções `DECODE_SCALAR` e `ENCODE_SCALAR` para alternar entre as formas de array de bytes e inteiro de um escalar.

As chaves públicas Red25519 são pontos na forma Edwards da Curve25519. Elas são codificadas como a representação little-endian de 255 bits da coordenada y, seguida por um único bit indicando o sinal da coordenada x. Esta é a mesma codificação usada para Ed25519. Definimos as funções `DECODE_POINT` e `ENCODE_POINT` para alternar entre o array de bytes e as formas de coordenadas de um ponto.

### Funções RedDSA

Para facilitar a implementação, escrevemos explicitamente abaixo as funções RedDSA (assim como várias funções auxiliares) já especializadas para Red25519. Os implementadores devem consultar a seção 5.4.6 de [RedDSA](https://github.com/zcash/zips/tree/master/protocol/protocol.pdf) para a especificação geral das funções RedDSA.
...
```
Note que a implementação de `CONVERT_ED25519_PRIVATE` é equivalente ao cálculo do escalar secreto s ao derivar uma chave pública Ed25519 de uma chave privada Ed25519, conforme especificado nos passos 1-3 da seção 5.1.5 do [RFC-8032](https://tools.ietf.org/html/rfc8032).

## Implicações de segurança

Regenerar aleatoriamente um Destination Red25519 e então criar assinaturas com ele não vaza nenhuma informação sobre o Destination, porque a distribuição de chaves privadas Red25519 geradas via `RANDOMIZE_PRIVATE` é idêntica à distribuição de chaves privadas geradas via `GENERATE_PRIVATE`, e `DERIVE_PUBLIC` é determinística.

Converter chaves privadas Ed25519 para Red25519 via `CONVERT_ED25519_PRIVATE` não resulta na mesma distribuição. No entanto, consideramos a redução na segurança aceitável pelas seguintes razões:

- O espaço dos escalares Ed25519 é aproximadamente metade do tamanho do espaço dos escalares Red25519 (existem 2^251 escalares Ed25519 possíveis, e L ~= 2^252 escalares Red25519 possíveis). Assim, a perda de segurança é no máximo um fator de aproximadamente 2, ou aproximadamente 1 bit (porque poderíamos ter coincidentemente escolhido um escalar Red25519 que também é um escalar Ed25519 válido).
- Destinos Ed25519 existentes já foram historicamente expostos na rede, e deve-se assumir que floodfills maliciosos já os enumeraram.

Usuários que estão preocupados com essa redução de segurança devem usar Red25519 como o sigtype para seus Destinations em vez de Ed25519.

Note que o argumento acima não se aplica ao escalar de re-randomização alpha; informações sobre a chave são vazadas cada vez que um alpha tendencioso é escolhido, porque a re-randomização aditiva se comporta como um one-time pad.

## Compatibilidade

As versões do I2P que suportam Red25519 poderão verificar estruturas de dados da rede assinadas com ele. As versões do I2P que não suportam Red25519 irão tratá-lo como uma assinatura desconhecida, e PODEM descartar as estruturas de dados. Os usuários devem esperar que a confiabilidade das estruturas de dados assinadas com Red25519 seja baixa até que seja alcançada uma implantação suficiente do Red25519.
...
- [Proposta 123](/proposals/123-new-netdb-entries)
