...
세션 요청(Session Request) 메시지의 긴 헤더에 있는 버전 필드는 PQ가 아닌 경우 2, MLKEM-512의 경우 3, MLKEM-768의 경우 4이다. 이를 통해 동일한 포트에서 표준 SSU2와 하이브리드 SSU2를 모두 실행하고 두 가지 MLKEM 변형을 동시에 지원할 수 있다.

## 라우터 호환성

### 전송 프로토콜 이름

모든 경우에 일반적으로 SSU2 전송 이름을 사용하세요. 이전 라우터는 pq 매개변수를 무시하고 기존과 같이 표준 SSU2로 연결합니다.
