...
description: "ML-KEM을 사용하는 NTCP2 전송 프로토콜의 양자 내성 하이브리드 변형"
slug: "ntcp2-hybrid"
lastupdated: "2026-02"
category: "전송 계층"
accurateFor: "0.9.69"
...
이것은 Proposal 169에서 설계된 NTCP2 전송 프로토콜의 하이브리드 포스트 양자 변형입니다. 추가적인 배경 정보는 해당 제안서를 참조하세요.

PQ Hybrid NTCP2는 표준 NTCP2와 동일한 주소와 포트에서만 정의됩니다. 다른 포트에서의 운영이나 표준 NTCP2 지원 없는 운영은 허용되지 않으며, 표준 NTCP2가 폐기되는 몇 년 후까지는 허용되지 않을 것입니다.

이 사양서는 PQ Hybrid를 지원하기 위해 표준 NTCP2에 필요한 변경사항만을 문서화합니다. 기본 구현 세부사항은 NTCP2 사양서를 참조하세요.

## 설계

우리는 NIST FIPS 203 및 204 표준 [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf) [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf)을 지원합니다. 이는 CRYSTALS-Kyber와 CRYSTALS-Dilithium (버전 3.1, 3, 및 이전 버전)을 기반으로 하지만 호환되지 않습니다.

### 키 교환

PQ KEM은 임시 키만 제공하며, Noise XK 및 IK와 같은 정적 키 핸드셰이크를 직접 지원하지 않습니다. 암호화 타입은 PQ Hybrid Ratchet에서 사용되는 것과 동일하며, [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf)에서와 같이 공통 구조 문서 [/docs/specs/common-structures/](/docs/specs/common-structures/)에 정의되어 있습니다. Hybrid 타입은 X25519와의 조합에서만 정의됩니다.
...
새로운 암호화 유형은 RouterAddress에서 표시됩니다. 키 인증서의 암호화 유형은 계속 유형 4가 됩니다.
...
하이브리드 전방 보안(hfs)을 위한 XK와 IK에 대한 다음 수정 사항들은 [Noise HFS spec](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf) 섹션 5에 명시된 대로입니다:
...
하이브리드 핸드셰이크는 [Noise HFS 사양](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf)에서 정의됩니다. Alice에서 Bob으로 전송되는 첫 번째 메시지는 메시지 페이로드 앞에 캡슐화 키인 e1을 포함합니다. 이것은 추가 정적 키로 취급됩니다. (Alice의 경우) EncryptAndHash()를 호출하거나 (Bob의 경우) DecryptAndHash()를 호출합니다. 그런 다음 메시지 페이로드를 평소와 같이 처리합니다.

Bob에서 Alice로의 두 번째 메시지는 메시지 페이로드 앞에 ekem1인 암호문을 포함합니다. 이는 추가적인 정적 키로 취급되며, (Bob으로서) EncryptAndHash()를 호출하거나 (Alice로서) DecryptAndHash()를 호출합니다. 그런 다음 kem_shared_key를 계산하고 MixKey(kem_shared_key)를 호출합니다. 그 후 평소와 같이 메시지 페이로드를 처리합니다.
...
encap_key와 ciphertext 모두 Noise handshake 메시지 1과 2의 ChaCha/Poly 블록 내에서 암호화된다는 점에 유의하세요. 이들은 handshake 과정의 일부로 복호화될 것입니다.

kem_shared_key는 MixHash()를 사용하여 chaining key에 혼합됩니다. 자세한 내용은 아래를 참조하세요.

#### 메시지 1에 대한 Alice KDF

'es' 메시지 패턴 이후, payload 이전에 다음을 추가하세요:
...
'es' 메시지 패턴 이후이고 페이로드 이전에 다음을 추가하세요:
...
변경사항: 현재 NTCP2는 ChaCha 섹션의 옵션들만 포함합니다. ML-KEM을 사용하면 ChaCha 섹션에는 암호화된 PQ 공개 키도 포함될 것입니다.

PQ와 non-PQ NTCP2가 같은 router 주소와 포트에서 지원될 수 있도록, X 값(X25519 임시 공개 키)의 최상위 비트를 사용하여 PQ 연결임을 표시합니다. 이 비트는 non-PQ 연결에서는 항상 설정되지 않습니다.

Alice의 경우, 메시지가 Noise에 의해 암호화된 후이지만 X의 AES 난독화 이전에 X[31] |= 0x7f를 설정합니다.

Bob의 경우, X의 AES 역난독화 후 X[31] & 0x80을 테스트합니다. 비트가 설정되어 있으면 X[31] &= 0x7f로 클리어하고 PQ 연결로 Noise를 통해 복호화합니다. 비트가 클리어되어 있으면 평소와 같이 비PQ 연결로 Noise를 통해 복호화합니다.

다른 router 주소와 포트에서 광고되는 PQ NTCP2의 경우, 이는 필요하지 않습니다.
...
<table style="border: 1px solid var(--color-border); border-collapse: collapse;">
<tr style="background-color: var(--color-bg-secondary);">
<th style="border: 1px solid var(--color-border); padding: 8px;">유형</th>
<th style="border: 1px solid var(--color-border); padding: 8px;">유형 코드</th>
...
</table>참고: 타입 코드는 내부 사용 전용입니다. router는 타입 4로 유지되며, 지원은 router 주소에 표시됩니다.
...
<table style="border: 1px solid var(--color-border); border-collapse: collapse;">
<tr style="background-color: var(--color-bg-secondary);">
<th style="border: 1px solid var(--color-border); padding: 8px;">타입</th>
<th style="border: 1px solid var(--color-border); padding: 8px;">타입 코드</th>
<th style="border: 1px solid var(--color-border); padding: 8px;">Y 길이</th>
<th style="border: 1px solid var(--color-border); padding: 8px;">Msg 2 길이</th>
<th style="border: 1px solid var(--color-border); padding: 8px;">Msg 2 암호화 길이</th>
<th style="border: 1px solid var(--color-border); padding: 8px;">Msg 2 복호화 길이</th>
<th style="border: 1px solid var(--color-border); padding: 8px;">PQ CT 길이</th>
<th style="border: 1px solid var(--color-border); padding: 8px;">opt 길이</th>
</tr>
<tr>
<td style="border: 1px solid var(--color-border); padding: 8px;">X25519</td>
<td style="border: 1px solid var(--color-border); padding: 8px;">4</td>
...
</table>참고: 타입 코드는 내부 사용 전용입니다. Router는 타입 4로 유지되며, 지원 여부는 router 주소에 표시됩니다.
...
모든 경우에 평소와 같이 NTCP2 transport 이름을 사용하세요.

비-PQ, 비-방화벽과 동일한 주소/포트를 사용합니다. 하나의 PQ 변형만 지원됩니다. router 주소에서 v=2(일반적으로)와 MLKEM 512/768/1024를 나타내는 새로운 매개변수 pq=[3|4|5]를 게시합니다. Alice는 세션 요청에서 임시 키의 MSB(key[31] & 0x80)를 설정하여 이것이 하이브리드 연결임을 나타냅니다. 위를 참조하세요. 구형 router들은 pq 매개변수를 무시하고 일반적인 방식으로 비-PQ 연결을 합니다.

다른 주소/포트를 PQ가 아닌 경우나 PQ 전용, 방화벽이 없는 경우로 사용하는 것은 지원되지 않습니다. 이는 PQ가 아닌 NTCP2가 비활성화되기까지 몇 년 후에 구현될 예정입니다. PQ가 아닌 방식이 비활성화되면 여러 PQ 변형이 지원될 수 있지만, 주소당 하나씩만 가능합니다. 지원될 때 router 주소에서 v=[3|4|5]를 게시하여 MLKEM 512/768/1024를 나타냅니다. Alice는 임시 키의 MSB를 설정하지 않습니다. 구형 router는 v 매개변수를 확인하고 지원되지 않는 주소로 건너뜁니다.

방화벽으로 차단된 주소 (IP가 공개되지 않음): router 주소에서 v=2를 게시합니다 (평소와 같이). pq 매개변수를 게시할 필요는 없습니다.

Alice는 자신의 router 정보에서 pq 지원을 광고하는지 여부나 동일한 변형을 광고하는지 여부에 관계없이, Bob이 게시하는 PQ 변형을 사용하여 PQ Bob에 연결할 수 있습니다.

#### 최대 패딩

현재 사양에서 메시지 1과 2는 "적당한" 양의 패딩을 갖도록 정의되어 있으며, 0-31바이트 범위가 권장되고 최대값은 명시되어 있지 않습니다.

API 0.9.68 (릴리스 2.11.0)까지 Java I2P는 non-PQ 연결에 대해 최대 256바이트 패딩을 구현했지만, 이는 이전에 문서화되지 않았습니다. API 0.9.69 (릴리스 2.12.0)부터 Java I2P는 non-PQ 연결에 대해 MLKEM-512와 동일한 최대 패딩을 구현합니다. 아래 표를 참조하세요.

정의된 메시지 크기를 최대 패딩으로 사용합니다. 즉, PQ 연결의 경우 최대 패딩이 메시지 크기를 두 배로 늘리게 됩니다:
...
NIST 보안 범주는 [NIST 프레젠테이션](https://www.nccoe.nist.gov/sites/default/files/2023-08/pqc-light-at-the-end-of-the-tunnel-presentation.pdf) 슬라이드 10에 요약되어 있습니다. 예비 기준: 하이브리드 프로토콜의 경우 최소 NIST 보안 범주는 2이고, PQ 전용의 경우 3이어야 합니다.

<table style="border: 1px solid var(--color-border); border-collapse: collapse;">
<tr style="background-color: var(--color-bg-secondary);">
<th style="border: 1px solid var(--color-border); padding: 8px;">카테고리</th>
<th style="border: 1px solid var(--color-border); padding: 8px;">보안 수준</th>
</tr>
<tr>
...
이들은 모두 하이브리드 프로토콜입니다. 구현체는 MLKEM768을 선호해야 합니다; MLKEM512는 충분히 안전하지 않습니다.

NIST 보안 카테고리 [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf):

<table style="border: 1px solid var(--color-border); border-collapse: collapse;">
<tr style="background-color: var(--color-bg-secondary);">
<th style="border: 1px solid var(--color-border); padding: 8px;">알고리즘</th>
<th style="border: 1px solid var(--color-border); padding: 8px;">보안 카테고리</th>
</tr>
<tr>
...
Bouncycastle, BoringSSL, 그리고 WolfSSL 라이브러리들이 이제 MLKEM과 MLDSA를 지원합니다. OpenSSL 지원은 2025년 4월 8일 3.5 릴리스에서 제공될 예정입니다 [OpenSSL](https://openssl-library.org/post/2025-02-04-release-announcement-3.5/).

### 인바운드 트래픽 식별

세션 요청에서 임시 키의 MSB (key[31] & 0x80)를 설정하여 이것이 하이브리드 연결임을 나타냅니다. 이를 통해 동일한 포트에서 표준 NTCP와 하이브리드 NTCP를 모두 실행할 수 있습니다. 인바운드에는 하나의 하이브리드 변형만 지원되며, router 주소에 광고됩니다. 예를 들어, pq=3 또는 pq=4입니다.

#### 난독화

Alice로서, PQ 연결의 경우, 난독화 이전에 X[31] |= 0x80을 설정합니다. 이는 X를 유효하지 않은 X25519 공개 키로 만듭니다. 난독화 후, AES-CBC가 이를 무작위화합니다. 난독화 후 X의 MSB는 무작위가 됩니다.

Bob으로서, 난독화 해제 후 (X[31] & 0x80) != 0인지 테스트합니다. 만약 그렇다면, 이것은 PQ 연결입니다.
...
모든 경우에 평소와 같이 NTCP2 transport 이름을 사용하십시오. 구형 router는 pq 매개변수를 무시하고 평소와 같이 표준 NTCP2로 연결합니다.
