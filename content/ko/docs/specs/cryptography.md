...
ElGamal 코드 ElGamalEngine을 참조하세요.
...
우리는 256비트 키와 128비트 블록을 가진 AES를 CBC 모드에서 사용합니다. 사용되는 패딩은 IETF [RFC-2313](http://tools.ietf.org/html/rfc2313) (PKCS#5 1.5, 섹션 8.1 (블록 타입 02용))에 명시되어 있습니다. 이 경우, 패딩은 16바이트 블록에 맞추기 위해 의사무작위로 생성된 옥텟으로 구성됩니다. 구체적으로, CBC 코드 CryptixAESEngine과 Cryptix AES 구현 CryptixRijndael_Algorithm, 그리고 ElGamalAESEngine.getPadding 함수 ElGamalAESEngine에서 찾을 수 있는 패딩을 참조하십시오.
...
서명은 DSAEngine에 구현된 1024 비트 [DSA](http://en.wikipedia.org/wiki/Digital_Signature_Algorithm) (L=1024, N=160)를 사용하여 생성되고 검증됩니다. DSA는 ElGamal보다 서명에서 훨씬 빠르기 때문에 선택되었습니다.
