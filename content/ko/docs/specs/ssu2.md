...
- 모든 헤더는 type = 2(세션 확인됨)입니다.
- 모든 헤더는 조각 번호와 전체 조각 수를 포함하는 "frag" 필드를 가집니다.
- 조각 0의 암호화되지 않은 헤더는 "거대(jumbo)" 메시지의 관련 데이터(AD)입니다.
- 각 헤더는 해당 패킷의 마지막 24바이트 데이터를 사용하여 암호화됩니다.
