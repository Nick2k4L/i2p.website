...
| 프로토콜 / 기능 | 상태 |
|--------------------|--------|
| Ratchet | Java I2P 및 i2pd에서 완료됨 |
| NTCP2 | 베타 버전 2026년 1분기, 출시 2026년 2분기 |
| SSU2 | 구현 진행 중, 베타 버전 2026년 2분기, 출시 2026년 3분기 |
| MLDSA SigTypes | 2027-2028년까지 보류 중, [PLANTS](https://datatracker.ietf.org/wg/plants/about/) 참조 |
## 개요
...
참고: 이 제안서에 포함된 MLDSA 서명과 관련된 모든 정보는 예비 단계에 있습니다. I2P에서의 MLDSA 서명 지원 작업은 2027년 후반 또는 2028년까지 보류 중이며, 이는 표준 기관들이 알고리즘을 선정하고, 키 및/또는 서명 크기를 축소하며, 산업계 채택을 촉진할 때까지 대기 중입니다. [CABFORUM](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/) 및 [PLANTS](https://datatracker.ietf.org/wg/plants/about/)를 참조하세요.

다음 구조들에서 PQ 및 하이브리드 서명을 지원할 것입니다:
...
| X.509 인증서 | 예 | 예 |
| Java keystores | 예 | 예 |
따라서 우리는 PQ 전용 서명과 하이브리드 서명 모두를 지원할 것입니다. [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf)에 따라 ML-DSA의 세 가지 변형을 정의하고, Ed25519와 결합한 세 가지 하이브리드 변형과 SU3 파일에만 사용되는 사전해시(prehash)를 포함한 세 가지 PQ 전용 변형을 추가하여 총 9가지의 새로운 서명 유형을 정의할 것입니다. 하이브리드 유형은 Ed25519와의 조합으로만 정의될 것입니다. SU3 파일을 제외하고는 표준 ML-DSA를 사용하며, 사전해시 변형인 HashML-DSA는 사용하지 않을 것입니다.

우리는 [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf) 3.4절에서 정의된 "결정적(deterministic)" 변형 대신, "헤지(hedged)" 또는 무작위화된 서명 변형을 사용할 것입니다. 이를 통해 동일한 데이터에 대해 서명하더라도 각 서명이 서로 달라지며, 사이드 채널 공격에 대한 추가적인 보호 기능을 제공합니다. 인코딩 및 컨텍스트를 포함한 알고리즘 선택에 대한 추가적인 내용은 아래의 구현 노트 섹션을 참조하세요.
...
| MLDSA65ph | 19 |
| MLDSA87ph | 20 |
X.509 인증서 및 기타 DER 인코딩은 [IETF 초안](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)에 정의된 복합 구조와 OID를 사용합니다.

오버헤드는 상당할 것이다. 일반적인 Ed25519 목적지 및 라우터 ID 크기는 391바이트이다. 이 크기는 사용하는 알고리즘에 따라 3.5배에서 6.8배까지 증가할 것이다. Ed25519 서명은 64바이트이며, 알고리즘에 따라 38배에서 76배까지 커진다. 일반적인 서명된 RouterInfo, LeaseSet, 응답 가능한 데이터그램, 서명된 스트리밍 메시지는 약 1KB 정도이다. 이들은 알고리즘에 따라 3배에서 8배까지 증가할 것이다.

새로운 대상지 및 라우터 신원 유형은 패딩을 포함하지 않으므로 압축이 불가능합니다. 전송 중에 gzip으로 압축된 대상지 및 라우터 신원의 크기는 사용하는 알고리즘에 따라 12배에서 38배까지 증가할 수 있습니다.

### 유효한 조합

대상(Destinations)의 경우, 새로운 서명 유형이 임대 세트(leaseset)의 모든 암호화 유형에서 지원됩니다. 키 인증서의 암호화 유형을 NONE(255)로 설정하세요.

RouterIdentity의 경우 ElGamal 암호화 유형은 더 이상 사용되지 않습니다. 새로운 서명 유형은 X25519(유형 4) 암호화에서만 지원됩니다. 새로운 암호화 유형은 RouterAddresses에 표시됩니다. 키 인증서의 암호화 유형은 계속해서 유형 4로 유지됩니다.
...
SHA3-256, SHAKE128 및 SHAKE256에 대한 테스트 벡터는 [NIST](https://csrc.nist.gov/projects/cryptographic-standards-and-guidelines/example-values)에서 확인할 수 있습니다.

Java BouncyCastle 라이브러리는 위의 모든 기능을 지원한다는 점에 유의하세요. C++ 라이브러리 지원은 OpenSSL 3.5 [OpenSSL](https://openssl-library.org/post/2025-02-04-release-announcement-3.5/)에서 제공됩니다.

### 대안

우리는 [FIPS 205](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.205.pdf) (Sphincs+)를 지원하지 않을 것입니다. 이는 ML-DSA보다 훨씬 느리고 크기 또한 큽니다. 다가올 FIPS 206(Falcon)도 지원하지 않을 것이며, 이는 아직 표준화되지 않았기 때문입니다. NIST에 의해 표준화되지 않은 NTRU나 기타 양자내성(PQ) 후보들도 지원하지 않을 것입니다.

### Rosenpass

와이어가드(Wireguard, IK)를 순수 양자내성 암호(PQ crypto)에 적용하는 것에 관한 일부 연구 [논문](https://eprint.iacr.org/2020/379.pdf)이 존재하지만, 해당 논문에는 해결되지 않은 여러 질문들이 있다. 이후 이 접근법은 양자내성 와이어가드(PQ Wireguard)를 위해 로젠패스(Rosenpass) [로젠패스](https://rosenpass.eu/) [화이트페이퍼](https://raw.githubusercontent.com/rosenpass/rosenpass/papers-pdf/whitepaper.pdf)로 구현되었다.

Rosenpass는 사전 공유된 Classic McEliece 460896 정적 키(각각 500KB)와 Kyber-512(본질적으로 MLKEM-512) 일시 키를 사용하는 Noise KK 유사 핸드셰이크를 사용합니다. Classic McEliece의 암호문은 단지 188바이트이며, Kyber-512의 공개키와 암호문 크기도 합리적이기 때문에 두 핸드셰이크 메시지 모두 표준 UDP MTU 내에 들어갑니다. 양자 안전(QP) KK 핸드셰이크에서 생성된 출력 공유 키(osk)는 표준 WireGuard IK 핸드셰이크를 위한 입력 사전 공유 키(psk)로 사용됩니다. 따라서 총 두 번의 완전한 핸드셰이크가 수행되며, 하나는 순수 양자 안전, 다른 하나는 순수 X25519 기반입니다.

다음과 같은 이유로 XK 및 IK 핸드셰이크를 대체하기 위해 이러한 조치들을 수행할 수 없습니다.

- KK를 수행할 수 없습니다. Bob이 Alice의 정적 키를 가지고 있지 않기 때문입니다
- 500KB 정적 키는 너무 큽니다
- 추가 왕복 통신은 원하지 않습니다

백서에는 유용한 정보가 많이 있으며, 우리는 아이디어와 영감을 얻기 위해 이를 검토할 것이다. TODO.

## 사양

### 공통 구조

다음과 같이 공용 구조 문서의 섹션과 표를 업데이트합니다 [/docs/specs/common-structures/](/docs/specs/common-structures/):

### PublicKey

새로운 공개 키(Public Key) 유형은 다음과 같습니다:
...
하이브리드 공개 키는 X25519 키입니다. KEM 공개 키는 앨리스가 밥에게 전송하는 일시적인 양자내성(PQ) 키입니다. 인코딩 및 바이트 순서는 [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf)에서 정의합니다.

MLKEM*_CT 키는 실제로 공개 키가 아니라, 노이즈 핸드셰이크에서 밥(Bob)이 앨리스(Alice)에게 전송하는 "암호문(ciphertext)"입니다. 여기에 이 키들을 완전성을 위해 나열합니다.
...
하이브리드 개인 키는 X25519 키입니다. KEM 개인 키는 앨리스 전용입니다. KEM 인코딩 및 바이트 순서는 [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf)에서 정의합니다.
...
비-PQ, 비-방화벽과 동일한 주소/포트를 사용합니다. PQ 변형 중 하나 또는 둘 다 지원됩니다. router 주소에서 v=2(평소대로)와 새로운 매개변수 pq=[3|4|3,4]를 게시하여 MLKEM 512/768/둘 다를 나타냅니다. 이전 router들은 pq 매개변수를 무시하고 평소대로 비-PQ로 연결합니다.
...
X25519 암호화 타입을 RIs에 사용한다고 가정할 때의 일반적인 키, 서명, RIdent, Dest 크기 또는 크기 증가량 (참조용으로 Ed25519 포함). Router Info, LeaseSet, 응답 가능한 데이터그램, 그리고 나열된 두 개의 스트리밍(SYN 및 SYN ACK) 패킷 각각에 대한 추가 크기. 현재 Destinations와 Leasesets은 반복되는 패딩을 포함하고 있어 전송 중 압축이 가능합니다. 새로운 타입들은 패딩을 포함하지 않으며 압축되지 않아, 전송 중 훨씬 높은 크기 증가를 초래합니다. 위의 설계 섹션을 참조하세요.
...
같은 터널에서 기존 ratchet 프로토콜과 새로운 ratchet 프로토콜을 모두 지원할 수 없다면, 마이그레이션이 훨씬 더 어려워질 것입니다.
