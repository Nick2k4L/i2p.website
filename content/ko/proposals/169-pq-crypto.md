...
변경 사항: 현재의 NTCP2는 단일 ChaCha 섹션 내의 옵션만 포함합니다. ML-KEM을 사용하면 옵션 이전에 새 ChaCha 섹션이 추가되며, 이 섹션에는 암호화된 양자내성(PQ) 공개 키가 포함됩니다.
...
변경 사항: 현재의 NTCP2는 단일 ChaCha 섹션 내의 옵션들만 포함합니다. ML-KEM을 사용하면 옵션 이전에 새로운 ChaCha 섹션이 추가되며, 이 섹션에는 암호화된 후양자(PQ) 암호문이 포함됩니다.
...
변경 사항: 현재의 SSU2는 단일 ChaCha 섹션 내에 블록 데이터만 포함합니다. ML-KEM을 사용하면 블록 데이터 이전에 새 ChaCha 섹션이 추가되며, 이 섹션에는 암호화된 양자내성(PQ) 공개 키가 포함됩니다.

스푸핑 방지를 위한 KDF 변경: Proposal 165 [Prop165]_에서 제기된 문제들을 해결하기 위해, 그러나 다른 해결책을 사용하여 PQ 세션에 한하여 세션 요청(Session Request)의 KDF를 수정합니다. 비-PQ 세션의 KDF는 변경되지 않습니다.
...
변경 사항: 현재의 SSU2는 단일 ChaCha 섹션에 페이로드만 포함합니다. ML-KEM을 사용하면 페이로드 이전에 새로운 ChaCha 섹션이 추가되어, 암호화된 후양자(PQ) 암호문을 포함하게 됩니다.
...
이들은 모두 하이브리드 프로토콜입니다. 구현 시에는 MLKEM768을 선호해야 하며, MLKEM512는 충분히 안전하지 않습니다.
...
현재 가장 우려되는 PQ 위협 모델은 오늘의 트래픽을 저장해두었다가 수십 년 후에 복호화하는 것입니다 (전진 보안성). 하이브리드 접근 방식이 이를 보호할 수 있을 것입니다.
