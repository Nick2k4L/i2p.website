...
| MLDSA65ph | 19 |
| MLDSA87ph | 20 |
X.509 인증서 및 기타 DER 인코딩은 [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)에 정의된 복합 구조와 OID를 사용합니다.
...
복합 하이브리드 서명 공개 키는 PQ 키 다음에 Ed25519 키가 오는 형태이며, 이는 [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)에서 정의한 바와 같습니다. 인코딩 및 바이트 순서는 [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf)에서 정의합니다.
...
복합 하이브리드 서명 개인 키는 [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)에서 명시한 바와 같이 PQ 키 다음에 Ed25519 키가 오는 형태입니다. 인코딩 방식과 바이트 순서는 [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf)에서 정의합니다.

서명용 개인 키는 절대로 외부로 전송되지 않습니다. 애플리케이션은 확장된 수 킬로바이트(KB) 크기의 개인 키 대신 [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)에서 권장하는 32비트 시드(seed) 형태로 저장할 수 있습니다. 이 방식은 구현 방식에 따라 달라질 수 있습니다.
...
복합 하이브리드 서명은 [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)에서 설명한 것처럼, PQ 서명 다음에 Ed25519 서명이 오는 형태입니다. 하이브리드 서명은 두 서명 모두를 검증하며, 어느 하나라도 검증에 실패하면 전체 검증이 실패합니다. 인코딩 및 바이트 순서는 [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf)에서 정의됩니다.
...
하이브리드 또는 PQ 서명 유형인 목적지의 경우 암호화 유형으로 NONE(유형 255)을 사용하지만, 암호화 키는 없으며 전체 384바이트의 주 섹션이 서명 키용으로 사용된다.
...
패딩 없음. 전체 길이는 7 + 전체 키 길이와 같다. 키 인증서 길이는 4 + 초과 키 길이이다.
...
MLDSA44를 위한 1351바이트 라우터 ID 바이트 스트림 예시:
...
합성 서명 알고리즘에 대한 새로운 명세를 다음과 같이 추가합니다: 복합 하이브리드 서명은 [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)에서 정의한 대로입니다. 그러나 일반적으로 I2P 내의 공개키와 서명은 DER 인코딩을 생략합니다.

복합 서명은 항상 사전 해싱을 사용하므로 잠재적으로 큰 메시지를 두 번 처리할 필요가 없습니다. 이것은 MLDSA 알고리즘 외부에서 이루어지며, HashML-DSA가 아닌 표준 MLDSA를 사용합니다.
...
서명 알고리즘과 동일함. 두 서명 중 하나라도 실패하면 전체가 실패함.
...
[COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)는 보안 강도의 불일치로 인해 MLDSA87 + Ed25519 조합을 정의하지 않습니다. 대신 MLDSA87 + Ed448 조합을 SHAKE256/64를 사전 해시 함수로 사용하여 정의하고 있습니다. 이 조합은 현재 Ed448를 지원하지 않기 때문에 본 제안서에 포함되어 있지 않습니다.

### 핸드셰이크 패턴

핸드셰이크는 [Noise 프로토콜](https://noiseprotocol.org/noise.html) 핸드셰이크 패턴을 사용합니다.
...
하이브리드 전방 보안(hfs)을 위한 XK 및 IK에 대한 다음 수정 사항은 [Noise HFS 사양](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf)의 5절에 명시된 대로입니다:
...
```
e1 패턴은 [Noise HFS 사양](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf)의 4절에 명시된 바와 같이 다음과 같이 정의된다:
...
For Alice:
  ...
  MixHash(ciphertext)
```
ekem1 패턴은 [Noise HFS 사양](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf)의 4절에서 명시된 바와 같이 다음과 같이 정의된다:
...
이 섹션은 IK 및 XK 프로토콜 모두에 적용됩니다.

하이브리드 핸드셰이크는 [Noise HFS 사양](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf)에서 정의된다. 앨리스에서 밥으로 가는 첫 번째 메시지는 메시지 페이로드 앞에 e1, 즉 캡슐화 키를 포함한다. 이것은 추가적인 정적 키로 취급되며, 앨리스 입장에서는 이를 EncryptAndHash()로 처리하고, 밥 입장에서는 DecryptAndHash()로 처리한다. 이후 메시지 페이로드는 일반적인 방식으로 처리한다.
...
우리는 [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf)에서 정의된 바와 같이 사용되는 암호화 구성 요소에 대응하는 다음 함수들을 정의한다.
...
encap_key와 암호문(ciphertext) 모두 Noise 핸드셰이크 메시지 1과 2 내부의 ChaCha/Poly 블록에서 암호화되어 있음에 유의하십시오. 이들은 핸드셰이크 과정의 일환으로 복호화됩니다.

kem_shared_key는 MixHash()를 통해 체이닝 키와 혼합됩니다. 자세한 내용은 아래를 참조하세요.

#### Message 1을 위한 Alice KDF

XK의 경우: 'es' 메시지 패턴 이후 페이로드 이전에 다음을 추가하세요:
...
다음과 같이 ECIES-Ratchet 사양 [/docs/specs/ecies/](/docs/specs/ecies/)을 업데이트합니다:
...
변경 사항: 기존 래칫은 첫 번째 ChaCha 섹션에 페이로드가 비어 있고, 두 번째 섹션에 페이로드가 포함되어 있었다. ML-KEM을 사용하면 이제 세 개의 섹션이 존재한다. 첫 번째 섹션은 암호화된 PQ 암호문을 포함하고, 두 번째 섹션은 페이로드가 비어 있으며, 세 번째 섹션에 페이로드가 포함된다.
...
변경 사항: 현재의 NTCP2은 단일 ChaCha 섹션에 있는 옵션들만 포함합니다. ML-KEM을 사용하면 옵션 이전에 새로운 ChaCha 섹션이 추가되며, 이 섹션에는 암호화된 양자내성(PQ) 공개 키가 포함됩니다.
...
I2P에서 MLDSA 서명 지원 작업은 표준 기관들이 알고리즘을 선정하고, 키 및/또는 서명 크기를 줄이는 방안을 마련하며, 산업계 채택을 촉진할 때까지 보류될 예정이며, 이는 2027년 후반 또는 2028년까지 지연될 수 있습니다. [CABFORUM](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/), [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/), 그리고 [PLANTS](https://datatracker.ietf.org/wg/plants/about/)를 참조하세요. 또한 MLDSA의 산업계 채택은 IETF, CA/Browser Forum, 그리고 인증 기관(CA)에 의해 표준화될 예정입니다. 인증 기관들은 먼저 하드웨어 보안 모듈(HSM) 지원이 필요하지만, 현재로서는 해당 지원이 제공되지 않고 있습니다 [CA/Browser Forum](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/). 우리는 IETF와 CA/Browser Forum이 복합 서명(composite signatures)의 지원 여부 또는 필수화 여부를 포함한 구체적인 파라미터 선택에 관한 결정을 주도할 것으로 기대하고 있습니다 [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/).
