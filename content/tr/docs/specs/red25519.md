...
description: "Körleştirilmiş Destination'lar oluşturmak için yeniden rastgeleleştirilebilir imza şeması"
slug: "red25519"
aliases:
  - "/tr/docs/specs/red25519-signature-scheme"
  - "/tr/docs/specs/red25519-signature-scheme/"
category: "Tasarım"
lastUpdated: "2020-08"
accurateFor: "0.9.47"
...
Bu belge, köreltilmiş Destination'lar oluşturmaya uygun yeniden randomize edilebilir bir imza şeması belirtir. Ayrıca mevcut Ed25519 Destination'ları köreltmek için de kullanılabilir, ancak etkinlikte hafif bir azalma ile.

## Motivasyon

[Öneri 123](/proposals/123-new-netdb-entries), En Az Yetki Prensibini benimseyen şifrelenmiş LeaseSet2 formatını tanımlar: her ağ katılımcısına sadece rolü için gerekli olan bilgi verilir. Özellikle, bir floodfill'e yayınlanan şifrelenmiş LeaseSet2, hangi Destination için olduğunu açığa çıkarmaz ve Lease'ler yalnızca Destination hakkında önceden bilgisi olan biri tarafından görüntülenebilir. Ancak, floodfill'lerin şifrelenmiş LeaseSet2'leri yayınlandıklarında doğrulayabilmesi gerekir ve istemcilerin ek olarak kimlik doğrulamanın Destination'ın kendisi tarafından zorlandığından emin olması gerekir.

Proposal 123 bunu Destination'ların imzalama anahtarlarını köreltme yoluyla gerçekleştirir. Köreltilmiş anahtarlar, floodfill'ler tarafından doğrulanabilir imzalar oluşturmak için kullanılabilir ve istemciler yalnızca Destination'ın bu imzaları oluşturmuş olabileceğinden emin olabilir. Bu nedenle köreltme için kullanılabilecek bir imza şeması belirtmek gereklidir.

## Tasarım

### Çekirdek imza şeması

Burada belirtilen imza şeması Red25519, anahtar yeniden rastgeleleştirmeyi destekleyen Schnorr tabanlı bir imza şeması olan [RedDSA](https://github.com/zcash/zips/tree/master/protocol/protocol.pdf)'nın bir uygulamasıdır. Aşağıdaki fonksiyonlara sahiptir:

**GENERATE_PRIVATE()** : Düzgün dağılımlı rastgele bir özel anahtar döndürür.

**DERIVE_PUBLIC(sk)** : Verilen özel anahtara karşılık gelen genel anahtarı döndürür.

**GENERATE_RANDOM()** : Bir anahtar çiftini yeniden rastgeleleştirmek için uygun rastgele bir skaler döndürür.
...
**SIGN(sk, m)** : Verilen mesaj m üzerinde sk özel anahtarı ile bir imza döndürür.

**VERIFY(vk, m, sig)** : İmza sig'i, genel anahtar vk ve mesaj m'ye karşı doğrular. İmza geçerliyse true, aksi takdirde false döndürür.

Verilen bir anahtar çifti (sk, vk) için aşağıdaki ilişki geçerlidir:
...
Ed25519 anahtarları, mevcut Ed25519 Destination'larının yeniden rastgeleleştirilmesini desteklemek amacıyla geçici olarak tek yönlü Red25519 anahtarlarına dönüştürülebilir. Diğer sigtype'lar uyumlu değildir.

Aşağıdaki dönüştürme fonksiyonlarını tanımlıyoruz:

**CONVERT_ED25519_PRIVATE(privkey)** : Verilen Ed25519 özel anahtarına karşılık gelen Red25519 özel anahtarını döndürür.

**CONVERT_ED25519_PUBLIC(pubkey)** : Verilen Ed25519 genel anahtarına karşılık gelen Red25519 genel anahtarını döndürür.

Belirli bir Ed25519 anahtar çifti (privkey, pubkey) için aşağıdaki ilişki geçerlidir:
...
**B** : [RFC-8032](https://tools.ietf.org/html/rfc8032)'de belirtildiği gibi Ed25519 temel noktası.

**L** : [RFC-8032](https://tools.ietf.org/html/rfc8032)'de belirtildiği gibi Ed25519 order 2^252 + 27742317777372353535851937790883648493.

**[s] B** : Temel noktanın s ile sabit-tabanlı skaler çarpımı.

**[s] A** : A'nın s ile değişken-tabanlı skaler çarpımı.

**x || y** : İki bayt dizisi x ve y'yi birleştir.

### Red25519

Red25519 şeması RedDSA'yı şunlarla özelleştirir:

- G := Curve25519'un Edwards formundaki noktaların grubu. Özellikle bu, Red25519'un L sırasındaki asal-sıralı alt grubu kullandığı ve kofaktör h_G'nin 8 olduğu anlamına gelir.
- P_G := Ed25519 taban noktası B.
- l_H := 512
- H(x) := SHA-512("I2P_Red25519H(x)" || x)

RedDSA, H(x)'in uzunluk genişletme saldırılarına karşı güvenli olan bir kriptografik hash fonksiyonu ile örneklendiğini varsayar. SHA-512 bunu kendi başına sağlamaz. Bunu düzeltmek için, mesajların uzunluklarının önek-serbest kodlaması ile öneklendirilmesini gerektiririz:

```
len_u16(M) || M
```
burada `len_u16(M)`, M'nin uzunluğunun 2-baytlık gösterimi olup, little-endian formatındadır (scalar'lar ve noktaların little-endian kodlamasıyla tutarlı olmak için).

Mesajlar 65534 bayttan uzun olmamalıdır. 65535 uzunluğu gelecekteki olası genişletmeler için ayrılmıştır.

### Kodlama ve kod çözme

Red25519 özel anahtarları, little-endian gösteriminde kodlanmış L modulo skalarlardır. Bir skaların bayt dizisi ve tamsayı formları arasında dönüşüm için `DECODE_SCALAR` ve `ENCODE_SCALAR` fonksiyonlarını tanımlıyoruz.

Red25519 public key'leri Curve25519'un Edwards formundaki noktalardır. Bunlar y-koordinatının 255-bit little-endian gösterimi olarak kodlanır ve ardından x-koordinatının işaretini gösteren tek bir bit gelir. Bu, Ed25519 ile aynı kodlamadır. Bir noktanın byte array ve koordinat formları arasında geçiş yapmak için `DECODE_POINT` ve `ENCODE_POINT` fonksiyonlarını tanımlıyoruz.

### RedDSA fonksiyonları

Uygulama kolaylığı için, aşağıda Red25519 için özelleştirilmiş RedDSA fonksiyonlarını (ve birkaç yardımcı fonksiyonu) açıkça yazıyoruz. Uygulayıcılar, RedDSA fonksiyonlarının genel spesifikasyonu için [RedDSA](https://github.com/zcash/zips/tree/master/protocol/protocol.pdf)'nın 5.4.6 bölümüne başvurmalıdır.
...
```
`CONVERT_ED25519_PRIVATE` uygulamasının, [RFC-8032](https://tools.ietf.org/html/rfc8032)'nin 5.1.5 bölümünden 1-3. adımlarda belirtildiği gibi, Ed25519 özel anahtarından Ed25519 genel anahtarı türetirken gizli skaler s'nin hesaplanmasına eşdeğer olduğuna dikkat edin.

## Güvenlik etkileri

Bir Red25519 Destination'ını yeniden rastgeleleştirmek ve ardından onunla imzalar oluşturmak, Destination hakkında herhangi bir bilgi sızdırmaz, çünkü `RANDOMIZE_PRIVATE` aracılığıyla üretilen Red25519 özel anahtarlarının dağılımı, `GENERATE_PRIVATE` aracılığıyla üretilen özel anahtarların dağılımı ile aynıdır ve `DERIVE_PUBLIC` deterministiktir.

Ed25519 özel anahtarlarını `CONVERT_ED25519_PRIVATE` ile Red25519'a dönüştürmek aynı dağılımla sonuçlanmaz. Ancak, aşağıdaki nedenlerle güvenlik azalmasını kabul edilebilir buluyoruz:

- Ed25519 skalarlarının uzayı, Red25519 skalarlarının uzayından yaklaşık yarı büyüklüktedir (2^251 olası Ed25519 skaları ve L ~= 2^252 olası Red25519 skaları vardır). Dolayısıyla güvenlik kaybı en fazla yaklaşık 2 faktörü veya yaklaşık 1 bit kadardır (çünkü tesadüfen aynı zamanda geçerli bir Ed25519 skaları olan bir Red25519 skalarını seçmiş olabiliriz).
- Mevcut Ed25519 Destination'lar ağda zaten tarihsel olarak maruz kalmıştır ve kötü niyetli floodfill'lerin bunları zaten numaralandırdığı varsayılmalıdır.

Bu güvenlik azalması konusunda endişeli olan kullanıcılar, Destination'ları için sigtype olarak Ed25519 yerine Red25519 kullanmalıdır.

Yukarıdaki argümanın yeniden rastgeleleştirme skaları alpha için geçerli olmadığını unutmayın; önyargılı bir alpha seçildiğinde anahtar hakkında bilgi sızar, çünkü toplama tabanlı yeniden rastgeleleştirme tek kullanımlık şifre gibi davranır.

## Uyumluluk

Red25519'u destekleyen I2P sürümleri, onunla imzalanmış ağ veri yapılarını doğrulayabilecektir. Red25519'u desteklemeyen I2P sürümleri bunu bilinmeyen bir imza olarak değerlendirecek ve veri yapılarını DÜŞÜREBİLİR. Kullanıcılar, Red25519'un yeterli dağıtımına ulaşılana kadar Red25519 ile imzalanmış veri yapılarının güvenilirliğinin düşük olmasını beklemeli.
