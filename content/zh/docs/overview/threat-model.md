...
- 对等节点的[严格排序](/docs/specs/implementation/#ordering)
- 对等节点分析和从缓慢变化的小组中选择
...
I2P通过四种方式避免了这个问题：首先，选择参与tunnel的peer不是在整个网络中随机采样的——它们来自peer选择算法，该算法将它们分为不同层级。其次，通过tunnel中peer的[严格排序](/docs/specs/implementation/#ordering)，peer出现频率更高的事实并不意味着它们是源头。第三，通过排列tunnel长度（默认未启用），即使是0跳tunnel也可以提供合理推诿性，因为网关的偶尔变化看起来像正常的tunnel。第四，通过受限路由（未实现），只有与目标有受限连接的peer才会联系目标，而攻击者只会遇到该网关。
