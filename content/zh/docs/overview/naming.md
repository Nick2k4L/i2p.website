...
查找不区分大小写。使用第一个匹配项，不会检测冲突。在查找中不强制执行命名规则。查找会缓存几分钟。Base 32 解析在[下文](#base32-names)中描述。有关命名服务 API 的完整描述，请参阅命名服务 Javadocs。该 API 在 0.8.7 版本中进行了显著扩展，提供了添加和删除功能、与主机名一起存储任意属性以及其他功能。
