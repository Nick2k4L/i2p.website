...
* 可使用路由器标识（router_ident）的签名公钥来验证该签名。

* 有关所有路由器信息中应包含的标准选项，请参阅网络数据库页面 [NETDB-ROUTERINFO](/docs/overview/network-database/#routerinfo)。

* 非常旧的路由器要求地址按照其数据的 SHA256 值进行排序，以确保签名不变。
  这一要求现已不再需要，且为向后兼容而实现也不值得。
