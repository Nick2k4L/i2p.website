---
title: "Blockfile 和主机数据库规范"
description: "I2P blockfile 文件格式规范以及 Blockfile Naming Service 使用的 hostsdb.blockfile 中表的规范"
slug: "blockfile"
category: "格式"
lastUpdated: "2023-11"
accurateFor: "0.9.59"
...
本文档规定了 I2P blockfile 文件格式以及 Blockfile 命名服务 [NAMING](/docs/overview/naming/) 使用的 hostsdb.blockfile 中的表格。
...
blockfile 是多个排序映射（键值对）的简单磁盘存储，实现为跳表。blockfile 格式采用自 Metanotion Blockfile Database [METANOTION](http://www.metanotion.net/software/sandbox/block.html)。首先我们将定义文件格式，然后说明 BlockfileNamingService 对该格式的使用。
...
**info** - 一个 Properties（UTF-8 字符串/字符串映射），序列化为 [Mapping](/docs/specs/common-structures/#type-mapping)：
...
- skiplist 键是 4 字节整数，为 [Destination](/docs/specs/common-structures/#struct-destination) 哈希值的前 4 个字节。
- skiplist 值分别是一个 Properties（UTF-8 字符串/字符串映射表），序列化为 [Mapping](/docs/specs/common-structures/#type-mapping)
...
**value** - - 数据库版本4：一个DestEntry，它是一个单字节数字，表示后续要跟随的Properties/Destination对的数量。该数量的对数包括：一个Properties（UTF-8字符串/字符串映射），序列化为[Mapping](/docs/specs/common-structures/#type-mapping)，后跟一个二进制[Destination](/docs/specs/common-structures/#struct-destination)（按常规方式序列化）。- 数据库版本3：一个DestEntry，它是一个Properties（UTF-8字符串/字符串映射），序列化为[Mapping](/docs/specs/common-structures/#type-mapping)，后跟一个二进制[Destination](/docs/specs/common-structures/#struct-destination)（按常规方式序列化）。
