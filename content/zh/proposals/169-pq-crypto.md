...
变更：当前的 NTCP2 仅在单个 ChaCha 段中包含选项。使用 ML-KEM 后，将在选项之前新增一个 ChaCha 段，其中包含已加密的抗量子（PQ）公钥。
...
变更：当前的 SSU2 仅在单个 ChaCha 段中包含块数据。使用 ML-KEM 后，将在块数据之前新增一个 ChaCha 段，用于包含加密的后量子（PQ）公钥。

用于防欺骗保护的 KDF 更改：为解决提案 165 [Prop165]_ 中提出的问题，我们采用不同的解决方案，即修改会话请求（Session Request）的 KDF。此更改仅适用于 PQ 会话，非 PQ 会话的 KDF 保持不变。
...
这些可能存在于带有较旧类型4 X25519密钥的leaseSet中。较旧的router将忽略未知的密钥。
