...
| MLDSA65ph | 19 |
| MLDSA87ph | 20 |
X.509 证书及其他 DER 编码将使用 [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/) 中定义的复合结构和对象标识符（OID）。
...
复合混合签名公钥由PQ密钥后跟Ed25519密钥组成，如[COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)中所述。编码和字节顺序在[FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf)中定义。
...
复合混合签名私钥由 PQ 密钥后跟 Ed25519 密钥组成，如 [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/) 中所定义。编码和字节顺序在 [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf) 中定义。

签名私钥绝不会通过网络传输。应用程序可以选择存储32字节的种子（如[COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)中所建议的），而不是展开后的多KB私钥。这取决于具体实现。
...
复合混合签名由PQ签名后跟Ed25519签名组成，如[COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)中所述。混合签名的验证需同时验证两个签名，若任意一个验证失败则整体失败。编码和字节顺序在[FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf)中定义。
...
对于使用混合型或PQ签名类型的地址，加密类型应使用NONE（类型255），但此时没有加密密钥，整个384字节的主区域都用于签名密钥。
...
无填充。总长度为 7 加上密钥总长度。密钥证书长度为 4 加上额外密钥长度。
...
添加复合签名算法的新规范，如下所示：复合混合签名的定义见 [COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)。但是，与通常情况一样，I2P 内的公钥和签名省略 DER 编码。

复合签名始终使用预哈希处理，因此潜在的大型消息无需被处理两次。这在MLDSA算法外部实现，我们使用的是标准MLDSA，而非HashML-DSA。
...
[COMPOSITE-SIGS](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/) 未定义 MLDSA87 + Ed25519 组合，可能是因为两者的安全强度不匹配。该文档确实定义了 MLDSA87 + Ed448 组合，使用 SHAKE256/64 作为预哈希函数。此组合目前未包含在本提案中，因为我们当前不支持 Ed448。

### 握手模式

握手使用 [Noise 协议](https://noiseprotocol.org/noise.html) 握手模式。
...
针对混合前向保密（hfs）的XK和IK的以下修改，如[Noise HFS 规范](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf)第5节所述：
...
```
e1 模式定义如下，具体见 [Noise HFS 规范](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf) 第 4 节：
...
For Alice:
  ...
  MixHash(ciphertext)
```
ekem1 模式的定义如下，具体见 [Noise HFS 规范](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf) 第 4 节：
...
混合握手在[Noise HFS 规范](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf)中定义。第一条消息由 Alice 发送给 Bob，其中包含消息载荷之前的一个封装密钥 e1。该密钥被视为一个额外的静态密钥；以 Alice 身份对其调用 EncryptAndHash()，或以 Bob 身份调用 DecryptAndHash()。然后像往常一样处理消息载荷。
...
我们定义了以下函数，对应于 [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf) 中定义的密码学基本构件。
...
请注意，encap_key 和密文都加密在 Noise 握手消息 1 和 2 的 ChaCha/Poly 数据块内，它们将在握手过程中被解密。

kem_shared_key 通过 MixHash() 混合到链式密钥中。详细信息请参见下文。

#### Alice KDF for Message 1

对于 XK：在 'es' 消息模式之后、有效载荷之前添加：
...
按照以下方式更新 ECIES-Ratchet 规范 [/docs/specs/ecies/](/docs/specs/ecies/)：
...
变更：当前的棘轮机制在第一个ChaCha段中具有空的有效载荷，有效载荷位于第二个段中。使用ML-KEM后，现在有三个段：第一个段包含加密的PQ密文，第二个段具有空的有效载荷，第三个段包含有效载荷。
...
变更：当前的 NTCP2 仅在单个 ChaCha 段中包含选项。使用 ML-KEM 后，将在选项之前新增一个 ChaCha 段，其中包含已加密的抗量子公钥。
