...
最大大小：使用 Bob 在其 RouterInfo 中公布的 MTU，如果 RouterInfo 中未提供，则使用默认值 1500。如果公布的 MTU 过低，请勿使用 MLKEM768_X25519。
...
最大大小：Alice 尚未获得 Bob 的 RouterInfo，因此不知道他公布的 MTU。对于此消息，按以下方式使用临时 MTU。对于 MLKEM512_X25519，将 1280 或收到的 SessionRequest 大小中的较大值作为 MTU。对于 MLKEM768_X25519，将（IPv4 为 1318 或 IPv6 为 1338）或收到的 SessionRequest 大小中的较大值作为 MTU。SessionCreated 的开销小于 SessionRequest 的开销，因为 MLKEM 密文比 MLKEM 公钥更短。这使得即使 SessionRequest 中几乎没有填充，SessionCreated 仍可使用一定范围的填充大小。
