...
- 我们是否应该停止发送0-RTT ratchet数据（除了LS之外）？
- 如果我们不发送0-RTT数据，我们是否应该将ratchet从IK切换到XK？
...
使用与非PQ、非防火墙相同的地址/端口。仅支持一种PQ变体。在router地址中，发布v=2（如往常一样）和新参数pq=[3|4|5]来表示MLKEM 512/768/1024。Alice在会话请求中设置临时密钥的最高位（key[31] & 0x80）来表示这是一个混合连接。见上文。较旧的router将忽略pq参数并像往常一样进行非pq连接。
...
讨论：将版本字段设置为 3 或 4 对于所有消息类型可能并非严格必要，但这样做有助于更早地检测到不支持的后量子连接失败。Token Request 和 Retry（类型 9 和 10）应该使用版本 3/4 以保持一致性。Hole Punch 消息（类型 11）可能不需要这种处理，但我们将遵循相同的模式以保持统一性。Peer Test 消息（类型 7）是会话外的，不表示启动会话的意图。
...
变更：当前的 SSU2 在 ChaCha 部分只包含区块数据。使用 ML-KEM 后，ChaCha 部分还将包含加密的 PQ 公钥。
...
变更：当前 SSU2 在 ChaCha 部分仅包含块数据。使用 ML-KEM 后，ChaCha 部分还将包含加密的 PQ 公钥。
...
不支持与非PQ使用不同地址/端口，或仅PQ、非防火墙的配置。这将不会被实现，直到非PQ SSU2被禁用，这需要几年时间。当非PQ被禁用时，支持一个或两个PQ变体。在router地址中，发布v=[3|4|3,4|4,3]来表示MLKEM 512/768/两者。较旧的router将检查v参数并跳过此地址作为不支持的地址。
...
[IETF 草案](https://datatracker.ietf.org/doc/draft-ietf-lamps-dilithium-certificates/) 第 8.1 节禁止在 X.509 证书中使用 HashML-DSA，并且不为 HashML-DSA 分配 OID，这是因为实现复杂性和安全性降低。

对于 SU3 文件的仅 PQ 签名，请使用 [IETF 草案](https://datatracker.ietf.org/doc/draft-ietf-lamps-dilithium-certificates/) 中定义的证书非预哈希变体的 OID。我们不定义 SU3 文件的混合签名，因为我们可能必须对文件进行两次哈希（尽管 HashML-DSA 和 X2559 使用相同的哈希函数 SHA512）。此外，在 X.509 证书中连接两个密钥和签名将完全不符合标准。
...
新的最大 Destination 大小将为 2599（base 64 编码后为 3468）。
...
典型的密钥、签名、RIdent、Dest 大小或大小增量（包含 Ed25519 作为参考），假设 RI 使用 X25519 加密类型。列出的是 Router Info、LeaseSet、可回复数据报以及两个流传输（SYN 和 SYN ACK）数据包各自的增加大小。当前的 Destinations 和 Leasesets 包含重复的填充，在传输过程中可压缩。新类型不包含填充且无法压缩，导致传输中的大小增量要高得多。请参阅上面的设计部分。
...
NIST 安全类别在 [NIST presentation](https://www.nccoe.nist.gov/sites/default/files/2023-08/pqc-light-at-the-end-of-the-tunnel-presentation.pdf) 第10页幻灯片中有总结。初步标准：我们的最低 NIST 安全类别对于混合协议应该是2，对于仅PQ（后量子）协议应该是3。
...
这个提案定义了混合和纯PQ签名类型。MLDSA44混合类型比MLDSA65纯PQ类型更可取。MLDSA65和MLDSA87的密钥和签名大小对我们来说可能太大了，至少在初期是这样。
...
虽然我们将定义并实现3种加密类型和9种签名类型，但我们计划在开发过程中测量性能，并进一步分析结构大小增加的影响。我们还将继续研究和监控其他项目和协议的发展。

经过开发和测试后，我们将为每个使用场景确定首选类型或默认选择。选择时需要在带宽、CPU和预估安全级别之间进行权衡。并非所有类型都适用于或允许用于所有使用场景。
...
签名：MLDSA87 和混合变体可能过大；MLDSA65 和混合变体可能过大

## 实现说明

### 库支持

Bouncycastle、BoringSSL 和 WolfSSL 库现在支持 MLKEM 和 MLDSA。OpenSSL 的支持将在 2025 年 4 月 8 日发布的 3.5 版本中提供 [OpenSSL](https://openssl-library.org/post/2025-02-04-release-announcement-3.5/)。

Java I2P 适配的 southernstorm.com Noise 库包含了对混合握手的初步支持，但我们因为未使用而将其移除；我们需要重新添加并更新以匹配 [Noise HFS spec](https://github.com/noiseprotocol/noise_hfs_spec/blob/master/output/noise_hfs.pdf)。

### 签名变体

我们将使用"对冲"或随机化签名变体，而非"确定性"变体，如 [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf) 第 3.4 节所定义。这确保即使对相同数据进行签名，每个签名都是不同的，并提供针对侧信道攻击的额外保护。虽然 [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf) 规定"对冲"变体是默认选择，但在各种库中这可能成立也可能不成立。实现者必须确保在签名时使用"对冲"变体。

我们使用标准的签名过程（称为纯 ML-DSA 签名生成），该过程在内部将消息编码为 0x00 || len(ctx) || ctx || message，其中 ctx 是大小为 0x00..0xFF 的可选值。我们没有使用任何可选上下文。len(ctx) == 0。此过程在 [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf) 算法 2 步骤 10 和算法 3 步骤 5 中定义。请注意，一些已发布的测试向量可能需要设置一种模式，在该模式下消息不会被编码。

### 可靠性

大小增加将导致 NetDB 存储、流握手和其他消息出现更多的 tunnel 分片。请检查性能和可靠性变化。
...
审查并可能减少存储在内存或磁盘中的最大 LS/RI 数量，以限制存储增长。提高 floodfill 的最低带宽要求？

### Ratchet

#### 共享 Tunnel

基于消息1（新会话消息）的长度检查，应该能够在同一tunnel上自动分类/检测多种协议。以MLKEM512_X25519为例，消息1的长度比当前的ratchet协议大816字节，最小消息1大小（仅包含DateTime负载）为919字节。当前ratchet的大多数消息1负载都小于816字节，因此可以将它们分类为非混合ratchet。大消息可能是POST请求，这种情况比较少见。

所以推荐的策略是：

- 如果消息1小于919字节，则使用当前的ratchet协议。
- 如果消息1大于或等于919字节，则可能是MLKEM512_X25519。
  先尝试MLKEM512_X25519，如果失败，则尝试当前的ratchet协议。

这应该允许我们在同一个目标上高效支持标准ratchet和混合ratchet，就像我们之前在同一个目标上支持ElGamal和ratchet一样。因此，我们可以比无法为同一个目标支持双协议的情况下更快地迁移到MLKEM混合协议，因为我们可以为现有目标添加MLKEM支持。
...
以下组合可能较为复杂，并非必须支持，但可能会根据具体实现而有所不同：
...
我们可能不会尝试在同一个目标上支持多种 MLKEM 算法（例如，MLKEM512_X25519 和 MLKEM_768_X25519）。只选择一种；但是，这取决于我们选择一个首选的 MLKEM 变体，以便 HTTP 客户端隧道可以使用。具体实现取决于实现方式。

我们可能会尝试在同一个目标上支持三种算法（例如 X25519、MLKEM512_X25519 和 MLKEM769_X25519）。分类和重试策略可能过于复杂。配置和配置界面可能过于复杂。这取决于具体实现。

我们可能不会尝试在同一个目标上同时支持ElGamal和混合算法。ElGamal已经过时，而且只支持ElGamal + 混合（没有X25519）意义不大。此外，ElGamal和混合新会话消息都很大，所以分类策略通常必须尝试两种解密方式，这会很低效。具体实现取决于实现方案。

客户端可以在相同的隧道上为X25519和混合协议使用相同或不同的X25519静态密钥，这取决于具体实现。

#### 前向保密性

ECIES 规范允许在新会话消息载荷中包含 Garlic Messages，这使得初始流数据包（通常是 HTTP GET）能够与客户端的 leaseSet 一起实现 0-RTT 传输。然而，新会话消息载荷不具备前向保密性。由于本提案强调增强 ratchet 的前向保密性，实现可能会或应当推迟包含流载荷或完整流消息，直到第一个现有会话消息。这将以牺牲 0-RTT 传输为代价。策略也可能取决于流量类型或 tunnel 类型，或者例如 GET 与 POST 的区别。具体实现相关。

#### 新会话大小

在同一目标上使用 MLKEM、MLDSA 或两者，将显著增加新会话消息的大小，如上所述。这可能会大大降低新会话消息通过 tunnel 传递的可靠性，因为它们必须被分片成多个 1024 字节的 tunnel 消息。传递成功率与分片数量成指数关系。实现可以使用各种策略来限制消息大小，但代价是牺牲 0-RTT 传递。具体取决于实现。

### NTCP2

我们在会话请求中设置临时密钥的最高有效位（key[31] & 0x80）来表示这是一个混合连接。这允许我们在同一端口上同时运行标准 NTCP 和混合 NTCP。只会支持一种混合变体，并在 router 地址中公布。例如，v=2,3 或 v=2,4 或 v=2,5。
...
我们在长头部中使用版本字段，对于 MLKEM512 将其设置为 3，对于 MLKEM768 将其设置为 4。地址中的 v=2,3,4 就足够了。
...
不推荐。仅使用上述与router类型匹配的新传输协议。旧版router无法连接、构建tunnel或发送netDb消息。在默认启用之前需要几个发布周期来调试和确保支持。相比下面的替代方案，可能会延长部署时间一年或更久。

#### Type 4 Router

推荐。由于PQ不影响X25519静态密钥或N握手协议，我们可以保持router为类型4，只需要广播新的传输方式。较旧的router仍然可以连接、构建tunnel通过或向其发送netDb消息。
...
较旧的 router 会验证 RI，因此无法连接、构建 tunnel 或发送 netDb 消息。在默认启用之前，需要几个发布周期来调试和确保支持。这将与加密类型 5/6/7 的部署面临相同的问题；可能会比上述列出的类型 4 加密类型部署替代方案延长一年或更多时间。
...
这些可能存在于带有较旧类型4 X25519密钥的leaseSet中。较旧的router会忽略未知的密钥。

目标节点可以支持多种密钥类型，但只能通过对消息1使用每个密钥进行试探性解密来实现。可以通过维护每个密钥成功解密次数的统计，并优先尝试使用频率最高的密钥来减轻开销。Java I2P在同一目标节点上对ElGamal+X25519使用了这种策略。

### 目标签名类型

#### 类型 12-17 目标

Router 会验证 leaseSet 签名，因此无法连接或接收类型 12-17 目标的 leaseSet。需要几个发布周期来调试并确保支持后才能默认启用。

没有替代方案。

## 优先级和推广

最有价值的数据是端到端流量，使用ratchet加密。作为tunnel跳跃之间的外部观察者，数据被额外加密了两次，分别是tunnel加密和传输加密。作为OBEP和IBGW之间的外部观察者，数据只被额外加密了一次，即传输加密。作为OBEP或IBGW参与者，ratchet是唯一的加密。然而，由于tunnel是单向的，要捕获ratchet握手中的两条消息需要router合谋，除非tunnel是在同一个router上构建OBEP和IBGW。

目前最令人担忧的后量子威胁模型是现在存储流量，然后在很多很多年后进行解密（前向保密）。混合方法可以防范这种威胁。

PQ威胁模型中在合理时间内（比如几个月内）破解认证密钥，然后伪装认证或近实时解密的威胁，还要更遥远一些？而那时我们才需要迁移到PQC静态密钥。

因此，最早的后量子威胁模型是 OBEP/IBGW 存储流量以便后续解密。我们应该首先实现混合 ratchet。

Ratchet 是最高优先级。传输协议是其次。签名是最低优先级。

签名推广也将比加密推广晚一年或更长时间，因为无法实现向后兼容。此外，MLDSA在行业中的采用将由CA/Browser Forum和证书颁发机构进行标准化。CA首先需要硬件安全模块(HSM)支持，但目前尚未提供[CA/Browser Forum](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/)。我们预计CA/Browser Forum将推动关于具体参数选择的决策，包括是否支持或要求复合签名[IETF draft](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)。
...
如果我们不能在相同的隧道上同时支持新旧棘轮协议，迁移将会困难得多。

我们应该能够像使用 X25519 时那样，先尝试一种方法，然后再尝试另一种方法，这样就能得到验证。

## 问题

- Noise Hash 选择 - 继续使用 SHA256 还是升级？
  SHA256 应该还能再用 20-30 年，不受 PQ 威胁，
