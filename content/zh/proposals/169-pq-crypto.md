...
| 协议 / 功能 | 状态 |
|--------------------|--------|
| 混合 MLKEM Ratchet 和 LS | 2025-06 批准；2025-08 进入测试版；2025-11 发布 |
| 混合 MLKEM NTCP2 | 已在主网测试，2026-02 批准；测试版目标 2026-02；发布目标 2026-05 |
| 混合 MLKEM SSU2 | 2026-02 批准；测试版目标 2026-05；发布目标 2026-08 |
| MLDSA 签名类型 12-14 | 初步阶段，暂停至 2027 年 |
| MLDSA 目标地址（Dests） | 初步阶段，暂停至 2027 年，已在主网测试，需网络升级以支持 floodfill |
| 混合签名类型 15-17 | 初步阶段，暂停至 2027 年 |
| 混合目标地址（Dests） | |
## 设计
...
签名功能的推出将比加密功能的推出晚一年或更长时间，因为无法实现向后兼容。

I2P 中对 MLDSA 签名的支持工作将暂停至 2027 年底或 2028 年，等待标准机构选定算法、可能缩小密钥和/或签名尺寸，并推动行业采用。详见 [CABFORUM](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/) 和 [PLANTS](https://datatracker.ietf.org/wg/plants/about/)。此外，MLDSA 在行业中的采用将由 CA/浏览器论坛（CA/Browser Forum）和证书颁发机构（CA）进行标准化。CA 首先需要硬件安全模块（HSM）的支持，而目前尚不可用 [CA/Browser Forum](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/)。我们预计 CA/浏览器论坛将主导关于具体参数选择的决策，包括是否支持或要求使用复合签名 [IETF draft](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/)。
...
如果我们无法在同一个 tunnel 上同时支持新旧 ratchet 协议，迁移将会困难得多。

我们应该能够像处理X25519时那样，先尝试一种然后再尝试另一种，这需要得到验证。
