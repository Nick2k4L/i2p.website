...
| 协议 / 功能 | 状态 |
|--------------------|--------|
| Ratchet | Java I2P 和 i2pd 中已完成 |
| NTCP2 | 2026 年第一季度进入测试版，2026 年第二季度发布 |
| SSU2 | 正在实现中，2026 年第二季度测试版，2026 年第三季度发布 |
| MLDSA SigTypes | 暂停至 2027-2028 年，详见 [PLANTS](https://datatracker.ietf.org/wg/plants/about/) |
## 概述
...
注意：本提案中有关 MLDSA 签名的所有信息均为初步内容。I2P 对 MLDSA 签名的支持工作已暂停，预计将在 2027 年底或 2028 年恢复，具体取决于标准机构对算法的选定工作，可能包括减小密钥和/或签名大小，以及推动行业采用。详见 [CABFORUM](https://cabforum.org/2024/10/10/2024-10-10-minutes-of-the-code-signing-certificate-working-group/) 和 [PLANTS](https://datatracker.ietf.org/wg/plants/about/)。
...
| X.509 证书 | 是 | 是 |
| Java keystores | 是 | 是 |
因此，我们将同时支持纯后量子（PQ-only）和混合签名（hybrid signatures）。我们将按照 [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf) 定义三种 ML-DSA 变体，三种与 Ed25519 结合的混合变体，以及三种仅用于 SU3 文件的带预哈希的 PQ-only 变体，共计新增 9 种签名类型。混合类型仅与 Ed25519 组合时定义。我们将使用标准的 ML-DSA，而不是带预哈希的变体（HashML-DSA），SU3 文件除外。

我们将使用“对冲”或随机化签名变体，而不是“确定性”变体，如 [FIPS 204](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.204.pdf) 第 3.4 节所定义。这确保了每次签名都不同，即使对相同数据进行签名，也能提供针对侧信道攻击的额外保护。有关算法选择（包括编码和上下文）的更多细节，请参见下文的实现说明部分。
...
| MLDSA65ph | 19 |
| MLDSA87ph | 20 |
X.509 证书及其他 DER 编码将使用 [IETF 草案](https://datatracker.ietf.org/doc/draft-ietf-lamps-pq-composite-sigs/) 中定义的复合结构和 OID。

开销将相当大。典型的 Ed25519 目标地址和路由器身份大小为 391 字节。根据算法不同，这些大小将增加 3.5 倍到 6.8 倍。Ed25519 签名为 64 字节，根据算法不同将增加 38 倍到 76 倍。典型的已签名 RouterInfo、LeaseSet、可回复数据报和已签名流消息大小约为 1KB，根据算法不同将增加 3 倍到 8 倍。

由于新的目标地址和路由器身份类型将不包含填充，因此无法进行压缩。在传输过程中经过 gzip 压缩的目的地和路由器身份的大小将增加 12 倍至 38 倍，具体取决于算法。

### 合法组合

对于目标地址（Destinations），新的签名类型在leaseset中支持所有加密类型。请将密钥证书中的加密类型设置为 NONE (255)。

对于 RouterIdentity，ElGamal 加密类型已被弃用。新的签名类型仅支持 X25519（类型 4）加密。新的加密类型将在 RouterAddress 中标明。密钥证书中的加密类型将继续为类型 4。
...
SHA3-256、SHAKE128 和 SHAKE256 的测试向量位于 [NIST](https://csrc.nist.gov/projects/cryptographic-standards-and-guidelines/example-values)。

请注意，Java 的 BouncyCastle 库支持上述所有功能。C++ 库支持已在 OpenSSL 3.5 中实现 [OpenSSL](https://openssl-library.org/post/2025-02-04-release-announcement-3.5/)。

### 替代方案

我们不会支持 [FIPS 205](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.205.pdf)（Sphincs+），它的速度要慢得多，且密钥尺寸远大于 ML-DSA。我们也不会支持即将发布的 FIPS 206（Falcon），因为它尚未标准化。我们不会支持 NTRU 或其他未被 NIST 标准化的后量子（PQ）候选算法。

### Rosenpass

有一篇研究[论文](https://eprint.iacr.org/2020/379.pdf)探讨了将 Wireguard（IK）适配用于纯后量子加密（PQ crypto），但该论文中仍存在若干未解决的问题。后来，这一方法被实现为用于后量子 Wireguard 的 Rosenpass [Rosenpass](https://rosenpass.eu/) [白皮书](https://raw.githubusercontent.com/rosenpass/rosenpass/papers-pdf/whitepaper.pdf)。

Rosenpass 使用一种类似于 Noise KK 的握手协议，结合预共享的 Classic McEliece 460896 静态密钥（每个 500 KB）和 Kyber-512（本质上是 MLKEM-512）临时密钥。由于 Classic McEliece 密文仅有 188 字节，而 Kyber-512 的公钥和密文大小也较为合理，因此两次握手消息均可容纳在标准 UDP MTU 内。抗量子（PQ）KK 握手生成的输出共享密钥（osk）将用作标准 WireGuard IK 握手中的输入预共享密钥（psk）。因此，总共包含两次完整的握手过程：一次完全基于抗量子算法，另一次完全基于 X25519。

我们不能用这些来替代我们的XK和IK握手，因为：

- 我们无法执行 KK，Bob 没有 Alice 的静态密钥
- 500KB 的静态密钥太大了
- 我们不想要额外的往返通信

白皮书中包含大量有价值的信息，我们将从中审阅并汲取想法和灵感。待办事项。
...
混合公钥是 X25519 密钥。KEM 公钥是 Alice 发送给 Bob 的临时后量子（PQ）密钥。编码和字节顺序在 [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf) 中定义。

MLKEM*_CT 密钥实际上不是公钥，它们是在 Noise 握手过程中由 Bob 发送给 Alice 的“密文”。此处列出是为了完整性。
...
混合私钥是 X25519 密钥。KEM 私钥仅用于 Alice。KEM 编码和字节顺序在 [FIPS 203](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf) 中定义。
...
目前最令人担忧的后量子威胁模型是存储今天的流量，以便在很多年后进行解密（前向保密）。混合方法可以防范这种威胁。
...
因此，最早的 PQ 威胁模型是 OBEP/IBGW 存储流量以供后续解密。我们应该首先实现混合棘轮。
