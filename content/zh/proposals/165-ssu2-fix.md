...
将 MixHash(Bob 的标识哈希)添加到“SessionRequest”消息的 NOISE 状态，例如： h = SHA256 (h || Bob 的标识哈希)。 它必须是用作 ENCRYPT 或 DECRYPT 的 ad 的最后一个 MixHash。 必须引入附加的 SSU2 标头标志“验证 Bob 的标识”= 0x02。
