---
title: "I2CP-Übersicht"
description: "Überblick über das I2P-Clientprotokoll (I2CP) – Sitzungsverwaltung, Optionen, Nutzdatenformat und Multiplexing."
slug: "i2cp-overview"
aliases: 
category: "Protokolle"
lastUpdated: "2025-04"
...
Einige Router- und Client-Implementierungen können auch externe Verbindungen über SSL unterstützen, wie durch die Option `i2cp.SSL=true` konfiguriert. Obwohl SSL nicht standardmäßig aktiviert ist, wird es dringend empfohlen, wenn der Datenverkehr dem offenen Internet ausgesetzt sein könnte. Der Autorisierungs-Benutzername/Passwort (falls vorhanden), der [Private Schlüssel](/docs/specs/common-structures/#type_PrivateKey) und der [Signierungs-Private-Schlüssel](/docs/specs/common-structures/#type_SigningPrivateKey) für das [Ziel (Destination)](/docs/specs/common-structures/#struct_Destination) werden alle unverschlüsselt übertragen, sofern SSL nicht aktiviert ist. Einige Router- und Client-Implementierungen können außerdem externe Verbindungen über Domain-Sockets unterstützen.

## I2CP-Protokollspezifikation

Siehe die [I2CP-Spezifikationsseite](/docs/specs/i2cp/) für die vollständige Protokollspezifikation.

## I2CP-Initialisierung {#initialization}

Wenn ein Client eine Verbindung zum Router herstellt, sendet er zunächst ein einzelnes Protokoll-Versionsbyte (0x2A). Danach sendet er eine [GetDate-Nachricht](/docs/specs/i2cp/#msg_GetDate) und wartet auf die Antwort in Form einer [SetDate-Nachricht](/docs/specs/i2cp/#msg_SetDate). Anschließend sendet er eine [CreateSession-Nachricht](/docs/specs/i2cp/#msg_CreateSession), die die Sitzungskonfiguration enthält. Danach wartet er auf eine [RequestLeaseSet-Nachricht](/docs/specs/i2cp/#msg_RequestLeaseSet) vom Router, die anzeigt, dass die eingehenden Tunnel erstellt wurden, und antwortet mit einer CreateLeaseSet-Nachricht, die das signierte LeaseSet enthält. Der Client kann nun Verbindungen zu anderen I2P-Zielen aufbauen oder von ihnen empfangen.

## I2CP-Optionen {#options}

### Routerseitige Optionen

Die folgenden Optionen werden traditionell über eine [SessionConfig](/docs/specs/i2cp/#struct_SessionConfig), die in einer [CreateSession-Nachricht](/docs/specs/i2cp/#msg_CreateSession) oder einer [ReconfigureSession-Nachricht](/docs/specs/i2cp/#msg_ReconfigureSession) enthalten ist, an den Router übergeben.
...
Hinweis: Hohe Mengen-, Längen- oder Varianzeinstellungen können erhebliche Leistungs- oder Zuverlässigkeitsprobleme verursachen.

Hinweis: Ab Version 0.7.7 müssen Optionennamen und -werte die UTF-8-Kodierung verwenden. Dies ist vor allem für Spitznamen nützlich. Vor dieser Version wurden Optionen mit mehrbyte-Zeichen beschädigt. Da Optionen in einer [Mapping](/docs/specs/common-structures/#type_Mapping) kodiert sind, sind alle Optionennamen und -werte auf maximal 255 Bytes (nicht Zeichen) begrenzt.
...
Hinweis: Alle Argumente, einschließlich Zahlen, sind Zeichenketten. Wahrheitswerte (true/false) sind nicht auf Groß-/Kleinschreibung empfindliche Zeichenketten. Alles, was nicht „true“ (unabhängig von der Groß-/Kleinschreibung) ist, wird als false interpretiert. Alle Optionennamen sind auf Groß-/Kleinschreibung empfindlich.

## I2CP-Nutzdatenformat und Multiplexing {#format}

Die von I2CP verarbeiteten Ende-zu-Ende-Nachrichten (d. h. die Daten, die vom Client in einer [SendMessageMessage](/docs/specs/i2cp/#msg_SendMessage) gesendet und vom Client in einer [MessagePayloadMessage](/docs/specs/i2cp/#msg_MessagePayload) empfangen werden), werden mit einem standardmäßigen 10-Byte-Gzip-Header komprimiert, der gemäß [RFC 1952](http://www.ietf.org/rfc/rfc1952.txt) mit 0x1F 0x8B 0x08 beginnt. Ab Version 0.7.1 nutzt I2P ungenutzte Teile des Gzip-Headers, um Protokoll-, From-Port- und To-Port-Informationen einzufügen, wodurch Streaming und Datagramme am selben Ziel unterstützt werden und Abfrage/Antwort mittels Datagrammen zuverlässig funktioniert, auch bei mehreren Kanälen.
...
Hinweis: Die I2P-Protokollnummern 224–254 sind für experimentelle Protokolle reserviert. Die I2P-Protokollnummer 255 ist für zukünftige Erweiterungen reserviert.

Die Datenintegrität wird mit dem standardmäßigen gzip CRC-32 gemäß [RFC 1952](http://www.ietf.org/rfc/rfc1952.txt) überprüft.

## Wichtige Unterschiede zum Standard-IP {#ip-differences}
